...
typedef std::function<cef_return_value_t(
    CefRefPtr<CefBrowser> browser,
    CefRefPtr<CefFrame> frame,
    CefRefPtr<CefRequest> request,
...
typedef std::function<bool(
    ...
    OnContextMenuCommandEvent;

//菜单消失（回调函数的调用线程：CEF的UI线程）
typedef std::function<void(CefRefPtr<CefBrowser> browser, CefRefPtr<CefFrame> frame)>
    OnContextMenuDismissedEvent;
...
typedef std::function<
    void(CefRefPtr<CefBrowser> browser, CefRefPtr<CefFrame> frame, const DString &url)>
    OnUrlChangeEvent;
...
typedef std::function<void(CefRefPtr<CefBrowser> browser, const std::vector<CefString> &icon_urls)>
    OnFaviconURLChangeEvent;

//全屏状态变化（回调函数的调用线程：主进程的UI线程）
typedef std::function<void(CefRefPtr<CefBrowser> browser, bool bFullscreen)>
    OnFullscreenModeChangeEvent;

//状态信息变化（回调函数的调用线程：主进程的UI线程）
typedef std::function<void(CefRefPtr<CefBrowser> browser, const DString &value)> OnStatusMessageEvent;

//加载进度变化（回调函数的调用线程：主进程的UI线程）
typedef std::function<void(CefRefPtr<CefBrowser> browser, double progress)>
    OnLoadingProgressChangeEvent;

//多媒体访问情况变化（回调函数的调用线程：主进程的UI线程）
typedef std::function<
    void(CefRefPtr<CefBrowser> browser, bool has_video_access, bool has_audio_access)>
    OnMediaAccessChangeEvent;
...
typedef std::function<
    bool(CefRefPtr<CefBrowser> browser, const CefString &url, const CefString &request_method)>
    OnCanDownloadEvent;
...
typedef std::function<void(
    ...
    OnDownloadFavIconFinishedEvent;
...
typedef std::function<void(
    ...
    OnDraggableRegionsChangedEvent;
