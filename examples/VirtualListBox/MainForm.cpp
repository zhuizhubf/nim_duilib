    ...
    if (args.eventType == ui::kEventSelect) {
        ...
        if (nOldItemIndex != ui::Box::InvalidIndex) {
            ...
            sInfo += ui::StringUtil::Printf(
                _T("NewItemIndex=%zu, NewElementID=%zu; OldItemIndex=%zu, OldElementID=%zu"),
                nNewItemIndex,
                nNewElementID,
                nOldItemIndex,
                nOldElementID);
            ...
            sInfo += ui::StringUtil::Printf(
                _T("NewItemIndex=%zu, NewElementID=%zu"), nNewItemIndex, nNewElementID);
        }
    } else if (args.eventType == ui::kEventSelChanged) {
        //无参数
    } else if (
        (args.eventType == ui::kEventItemMouseEnter) || (args.eventType == ui::kEventItemMouseLeave)
        || (args.eventType == ui::kEventMouseDoubleClick) || (args.eventType == ui::kEventClick)
        || (args.eventType == ui::kEventRClick) || (args.eventType == ui::kEventReturn)) {
        ...
        if (nItemIndex == ui::Box::InvalidIndex) {
            ...
            sInfo
                += ui::StringUtil::Printf(_T("ItemIndex=%zu, ElementID=%zu"), nItemIndex, nElementID);
        }
        ...
        ASSERT(nElementID == nCalcElementID);
        sInfo += ui::StringUtil::Printf(
            _T("ItemIndex=%zu, ElementID=%zu, ListBoxItem: 0x%p"),
            nItemIndex,
            nElementID,
            args.pEventData);
        ui::IListBoxItem *pListBoxItem = dynamic_cast<ui::IListBoxItem *>(
            (ui::Control *) args.pEventData);
