...
void MainForm::OnInitWindow()
{
    ...
    ui::Button *pOpenNewWindowDark = dynamic_cast<ui::Button *>(
        FindControl(_T("open_new_window_as_dark")));
    ...
    ui::Button *pOpenNewWindowLight = dynamic_cast<ui::Button *>(
        FindControl(_T("open_new_window_as_light")));
    ...
    //主题生成相关
    ui::CheckBox *pGenColorCheckBox = dynamic_cast<ui::CheckBox *>(
        FindControl(_T("gen_color_checkbox")));
    ...
    ui::Slider *pSliderBaseChroma = dynamic_cast<ui::Slider *>(
        FindControl(_T("gen_color_slider_base_chroma")));
    ui::Label *pLabelBaseChroma = dynamic_cast<ui::Label *>(
        FindControl(_T("gen_color_label_base_chroma")));
    if ((pGenColorCheckBox == nullptr) || (pGenColorBox == nullptr) || (pSliderHue == nullptr)
        || (pLabelHue == nullptr) || (pSliderBaseChroma == nullptr)
        || (pLabelBaseChroma == nullptr)) {
        return;
    }
    ...
    ui::Option *pOptionColorType = dynamic_cast<ui::Option *>(
        FindControl(_T("gen_color_radio_light")));
    ...
    //仅使用中性色
    ui::CheckBox *pGenNeutralColorCheckBox = dynamic_cast<ui::CheckBox *>(
        FindControl(_T("gen_color_neutral_mode")));
...
void MainForm::OnGenColorParamChanged()
{
    ...
    ui::Label *pLabelBaseChroma = dynamic_cast<ui::Label *>(
        FindControl(_T("gen_color_label_base_chroma")));
    if ((pSliderHue == nullptr) || (pLabelHue == nullptr) || (pSliderBaseChroma == nullptr)
        || (pLabelBaseChroma == nullptr)) {
        return;
    }
    ...
    if (m_pThemeGenerator == nullptr) {
        //从当前主题加载默认的配置
        ui::FilePath globalXmlFileName = ui::FilePath(
            ui::GlobalManager::Instance().Theme().GetGlobalXmlFileName());
        ...
        themeFullPath
            /= ui::GlobalManager::Instance().Theme().GetCurrentColorThemeInfo().m_themePath;
    ...
    ui::Option *pOptionColorType = dynamic_cast<ui::Option *>(
        FindControl(_T("gen_color_radio_dark")));
    ...
    previewColors.push_back(
        std::make_pair<DString, DString>(_T("--background"), _T("--foreground")));
    previewColors.push_back(
        std::make_pair<DString, DString>(_T("--accent"), _T("--accent_foreground")));
    ...
    for (auto iter : previewColors) {
        ...
        if (pLabel != nullptr) {
            std::string bkColor = m_pThemeGenerator->GetGeneratedColor(
                ui::StringConvert::TToUTF8(bg));
            pLabel->SetBkColor(ui::StringConvert::UTF8ToT(bkColor));
            pLabel->SetToolTipText(ui::StringConvert::UTF8ToT(bkColor));
            if (!fg.empty()) {
                std::string fgColor = m_pThemeGenerator->GetGeneratedColor(
                    ui::StringConvert::TToUTF8(fg));
                pLabel->SetStateTextColor(
                    ui::ControlStateType::kControlStateNormal, ui::StringConvert::UTF8ToT(fgColor));
            }
        }
    }
}
...
void MainForm::ShowAllColors()
{
    ...
    //所有颜色
    std::map<std::string, ui::ThemeColorConfig> themeColorConfigs
        = themeGenerator.GetLoadedConfigs();
