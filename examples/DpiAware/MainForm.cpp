...
void MainForm::OnInitWindow()
{
    ...
    if (pButton != nullptr) {
        pButton->AttachClick([this](const ui::EventArgs & /*args*/) {
            //修改界面显示比例
            ui::RichEdit *pRichEdit = dynamic_cast<ui::RichEdit *>(
                FindControl(_T("display_scale_factor")));
...
void MainForm::UpdateUI()
{
    ...
    if ((pLabel != nullptr) && (pGroupTest != nullptr)) {
        DString text = ui::StringUtil::Printf(
            _T("[left: %d, top: %d]"), pGroupTest->GetRect().left, pGroupTest->GetRect().top);
        pLabel->SetText(text);
    }
    ...
    if (pLabel != nullptr) {
        ...
        DString text
            = ui::StringUtil::Printf(_T("%d%% (DisplayScale: %.02f)"), nScaleFactor, fScale);
        pLabel->SetText(text);
    }
