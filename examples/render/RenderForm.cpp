...
void RenderForm::OnInitWindow()
{
    ...
    if (pMenuBar != nullptr) {
        pMenuBar->AddTopMenu(
            ...
            _T("File"),
            _T(""),
            DString(DUILIB_PUBLIC_RES_DIR) + _T("/menu/rich_edit_menu.xml"));
...
{
    ui::PropertyGrid *pPropertyGrid = dynamic_cast<ui::PropertyGrid *>(
        FindControl(_T("property_grid_test")));
    if (pPropertyGrid == nullptr) {
        return;
    }
    // 表头文本
    pPropertyGrid->SetEnableHeaderCtrl(
        true,
    ...
    auto p = pPropertyGrid->AddTextPropertyById(
        pGroup,
    ...
    auto p00 = pPropertyGrid->AddTextPropertyById(
        pGroup,
    ...
    pPropertyGrid->AddTextPropertyById(
        pGroup,
        _T("STRID_RENDER_PROPGRID_PROPERTY_2"),       // 属性2
        _T("STRID_RENDER_PROPGRID_VALUE_2"),          // 值2
    ...
    auto p0 = pPropertyGrid->AddTextPropertyById(
        pGroup,
    ...
    auto p1 = pPropertyGrid->AddTextPropertyById(
        pGroup,
    ...
    auto p2 = pPropertyGrid->AddTextPropertyById(
        pGroup,
    ...
    auto p3 = pPropertyGrid->AddTextPropertyById(
        pGroup,
    ...
    auto p10 = pPropertyGrid->AddComboPropertyById(
        pGroup,
    ...
    auto p11 = pPropertyGrid->AddComboPropertyById(
        pGroup,
    ...
    auto p20 = pPropertyGrid->AddFontPropertyById(
        pGroup,
        ...
        0,
        false);
    auto p21 = pPropertyGrid->AddFontSizePropertyById(
        pGroup,
    ...
    auto p22 = pPropertyGrid->AddColorPropertyById(
        pGroup,
    ...
    pPropertyGrid->AddDateTimePropertyById(
        pGroup,
        ...
        0,
        false,
        ui::DateTime::EditFormat::kDateCalendar); // 描述信息：设置日期
    pPropertyGrid->AddDateTimePropertyById(
        pGroup,
        _T("STRID_RENDER_PROPGRID_DATE"), // 日期
        _T("2023/12/07"),
    ...
    pPropertyGrid->AddIPAddressPropertyById(
        pGroup,
        ...
        0,
        false);
    pPropertyGrid->AddHotKeyPropertyById(
        pGroup,
    ...
    auto p80 = pPropertyGrid->AddFilePropertyById(
        pGroup,
        ...
        0,
        _T("txt"));
    auto p81 = pPropertyGrid->AddFilePropertyById(
        pGroup,
        _T("STRID_RENDER_PROPGRID_FILE_PATH"), // 文件路径
        _T("C:\\Test-Open.txt"),
        _T("STRID_RENDER_PROPGRID_DESC_FILE_PATH"), // 描述信息：文件路径
    ...
    auto p82 = pPropertyGrid->AddDirectoryPropertyById(
        pGroup,
        _T("STRID_RENDER_PROPGRID_FOLDER"), // 文件夹
        _T("C:\\Test\\"),
        _T("STRID_RENDER_PROPGRID_DESC_FOLDER"),
        0,
        false); // 描述信息：文件夹
}
