...
// 定义图形类型枚举
typedef enum { SHAPE_RECTANGLE, SHAPE_CIRCLE, SHAPE_LINE, SHAPE_COUNT } ShapeType;
...
public:
    ...
    */
    void PaintChildWindowLayered(
        HWND hWnd, const ui::UiRect &rcPaint, const ui::UiRect &rcChildWindow, bool bPaintFps);
...
HBITMAP ChildWindowPaint::TImpl::CreateHBitmap(
    HWND hWnd, int32_t nWidth, int32_t nHeight, bool flipHeight) const
{
...
{
    if (m_cacheBitmaps.empty() || m_cacheBitmaps[0].width != width
    ...
    BitBlt(hdc, 0, 0, width, height, m_cacheBitmaps[nIndex].hMemDC, 0, 0, SRCCOPY);

    return true;
}

void ChildWindowPaint::TImpl::PaintChildWindow(
    HWND hWnd, const ui::UiRect & /*rcPaint*/, bool bPaintFps)
{
    // 获取窗口客户区尺寸
    RECT rectClient;
...
void ChildWindowPaint::TImpl::PaintChildWindowLayered(
    HWND hWnd, const ui::UiRect & /*rcPaint*/, const ui::UiRect &rcChildWindow, bool bPaintFps)
{
    // 获取窗口客户区尺寸
    RECT rcClient;
    ...
    bool bRet
        = ::UpdateLayeredWindow(hWnd, nullptr, &pt, &szWindow, hPaintDC, &ptSrc, 0, &bf, ULW_ALPHA)
          != FALSE;
...
void ChildWindowPaint::PaintChildWindow(
    const ui::UiRect &rcPaint, const ui::NativeMsg &nativeMsg, bool bPaintFps)
{
