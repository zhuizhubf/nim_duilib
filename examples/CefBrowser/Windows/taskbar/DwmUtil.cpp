...
HRESULT DwmSetIconicThumbnail(HWND hwnd, HBITMAP hbmp, DWORD dwSITFlags)
{
    ...
    if (pDwmSetIconicThumbnail) {
        ...
        if (hModule != nullptr) {
            pDwmSetIconicThumbnail
                = (LPSetProcessDPIAware) GetProcAddress(hModule, "DwmSetIconicThumbnail");
            if (pDwmSetIconicThumbnail) {
                return pDwmSetIconicThumbnail(hwnd, hbmp, dwSITFlags);
            }
        }
...
{
    typedef HRESULT(WINAPI * LPDwmSetIconicLivePreviewBitmap)(
    ...
    if (pDwmSetIconicLivePreviewBitmap) {
        ...
        HMODULE hModule = ui::DllManager::Instance().LoadDll(_T("dwmapi.dll"));
        if (hModule != nullptr) {
            pDwmSetIconicLivePreviewBitmap = (LPDwmSetIconicLivePreviewBitmap)
                GetProcAddress(hModule, "DwmSetIconicLivePreviewBitmap");
            if (pDwmSetIconicLivePreviewBitmap) {
                return pDwmSetIconicLivePreviewBitmap(hwnd, hbmp, pptClient, dwSITFlags);
            }
        }
...
HRESULT DwmInvalidateIconicBitmaps(HWND hwnd)
{
    ...
    if (pDwmInvalidateIconicBitmaps) {
        ...
        HMODULE hModule = ui::DllManager::Instance().LoadDll(_T("dwmapi.dll"));
        if (hModule != nullptr) {
            pDwmInvalidateIconicBitmaps = (LPDwmInvalidateIconicBitmaps)
                GetProcAddress(hModule, "DwmInvalidateIconicBitmaps");
            if (pDwmInvalidateIconicBitmaps) {
                return pDwmInvalidateIconicBitmaps(hwnd);
            }
        }
...
{
    typedef HRESULT(WINAPI * LPDwmSetWindowAttribute)(
    ...
    if (pDwmSetWindowAttribute) {
        ...
        if (hModule != nullptr) {
            pDwmSetWindowAttribute
                = (LPDwmSetWindowAttribute) GetProcAddress(hModule, "DwmSetWindowAttribute");
            if (pDwmSetWindowAttribute) {
