...
void CefForm::OnInitWindow()
{
    ...
    GetRoot()->AttachBubbledEvent(
        ui::kEventClick, UiBind(&CefForm::OnClicked, this, std::placeholders::_1), 0);
    ...
    if (m_pCefControl != nullptr) {
        ...
        m_pCefControl->AttachMainUrlChange(
            UiBind(&CefForm::OnMainUrlChange, this, std::placeholders::_1, std::placeholders::_2));
    }

    if (m_pCefControl != nullptr) {
        m_pCefControl->AttachDevToolAttachedStateChange(UiBind(
            &CefForm::OnDevToolVisibleStateChanged,
            this,
            std::placeholders::_1,
            std::placeholders::_2));
    }
    ...
    ui::CefManager::GetInstance()->SetAlreadyRunningAppRelaunch(
        UiBind(&CefForm::OnAlreadyRunningAppRelaunch, this, std::placeholders::_1));
    ...
    DString logMsg = ui::StringUtil::Printf(
        _T("[SDL: VideoDriver:\"%s\", RenderName:\"%s\"]"), driverName.c_str(), renderName.c_str());
...
void CefForm::OnAlreadyRunningAppRelaunch(const std::vector<DString> &argumentList)
{
    if (ui::GlobalManager::Instance().IsInUIThread()) {
        ...
        ui::GlobalManager::Instance().Thread().PostTask(
            ui::kThreadUI, UiBind(&CefForm::OnAlreadyRunningAppRelaunch, this, argumentList));
    }
}
...
void CefForm::OnBeforeContextMenu(
...
bool CefForm::OnContextMenuCommand(
    CefRefPtr<CefBrowser> browser,
    CefRefPtr<CefFrame> frame,
    CefRefPtr<CefContextMenuParams> params,
    int command_id,
...
void CefForm::OnFaviconURLChange(
...
void CefForm::OnDraggableRegionsChanged(
    CefRefPtr<CefBrowser> browser,
    CefRefPtr<CefFrame> frame,
...
cef_return_value_t CefForm::OnBeforeResourceLoad(
    CefRefPtr<CefBrowser> browser,
    CefRefPtr<CefFrame> frame,
    CefRefPtr<CefRequest> request,
...
void CefForm::OnResourceRedirect(
    CefRefPtr<CefBrowser> browser,
    CefRefPtr<CefFrame> frame,
    CefRefPtr<CefRequest> request,
...
bool CefForm::OnResourceResponse(
    CefRefPtr<CefBrowser> browser,
    CefRefPtr<CefFrame> frame,
    CefRefPtr<CefRequest> request,
    CefRefPtr<CefResponse> response)
...
void CefForm::OnResourceLoadComplete(
    CefRefPtr<CefBrowser> browser,
    CefRefPtr<CefFrame> frame,
    CefRefPtr<CefRequest> request,
    CefRefPtr<CefResponse> response,
...
void CefForm::OnProtocolExecution(
    CefRefPtr<CefBrowser> browser,
    CefRefPtr<CefFrame> frame,
    CefRefPtr<CefRequest> request,
...
void CefForm::OnLoadingStateChange(
        ...
        m_pCefControl->RegisterCppFunc(
            ...
            ToWeakCallback([this](const std::string &params, ui::ReportResultFunction callback) {
                ...
                ui::SystemUtil::ShowMessageBox(
                    this, value.c_str(), _T("C++ receives the message sent by JavaScript."));
                callback(false, R"({ "message": "Success." })");
            }));

        //接收网页端的文件拖入操作
        m_pCefControl->RegisterCppFunc(
            _T("OnDropFilesToBrowser"),
            ToWeakCallback([this](const std::string &params, ui::ReportResultFunction callback) {
...
void CefForm::OnDropFiles(const DString &jsonDropFileList)
{
    ...
    ui::SystemUtil::ShowMessageBox(
        this,
        jsonDropFileList.c_str(),
        _T("CefForm::OnDropFiles: C++ receives the message sent by JavaScript."));
    //业务处理
...
void CefForm::OnDownloadUpdated(
    CefRefPtr<CefBrowser> browser,
    CefRefPtr<CefDownloadItem> download_item,
...
void CefForm::OnDownloadFavIconFinished(
    CefRefPtr<CefBrowser> browser,
