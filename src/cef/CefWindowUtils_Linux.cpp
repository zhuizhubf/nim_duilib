...
public:
    virtual void Execute() override
    {
        ...
        if ((cefWindow != 0) && (cefDisplay != nullptr) && (hParentWindow != 0)) {
            if (IsX11WindowValid(cefDisplay, cefWindow)
                && IsX11WindowValid(cefDisplay, hParentWindow)) {
                UiRect rc = m_pCefControl->GetPos();
                XErrorHandler old_handler = XSetErrorHandler(
