...
{
    GetStorageForBytes(
        stgmed, data.c_str(), (data.size() + 1) * sizeof(typename std::basic_string<T>::value_type));
}

void GetStorageForFileDescriptor(STGMEDIUM *storage, const DString &file_name)
{
    ...
    if (tag_start == std::string::npos) {
        ...
        if (start_fragment_start != std::string::npos) {
            *fragment_start = static_cast<size_t>(
                atoi(cf_html.c_str() + start_fragment_start + start_fragment_str.length()));
        }
        ...
        if (end_fragment_start != std::string::npos) {
            *fragment_end = static_cast<size_t>(
                atoi(cf_html.c_str() + end_fragment_start + end_fragment_str.length()));
        }
    } else {
...
void CFHtmlToHtml(const std::string &cf_html, std::string *html, std::string *base_url)
{
...
{
    static DWORD file_desc_format = ::RegisterClipboardFormat(CFSTR_FILEDESCRIPTOR);
    return file_desc_format;
}

DWORD GetFileContentsFormat()
{
    static DWORD file_contents_format = ::RegisterClipboardFormat(CFSTR_FILECONTENTS);
    return file_contents_format;
}

bool DragDataToDataObject(CefRefPtr<CefDragData> drag_data, IDataObject **data_object)
{
    ...
    if (drag_data->IsLink() && !drag_data->GetLinkURL().empty()) {
        ...
        fmtetc.cfFormat = (CLIPFORMAT) moz_url_format;
        fmtetcs[curr_index] = fmtetc;
    ...
    if (!html.empty()) {
        ...
        fmtetc.cfFormat = (CLIPFORMAT) html_format;
        fmtetcs[curr_index] = fmtetc;
    ...
    if (bufferSize) {
        ...
        curr_index++;
        GetStorageForBytes(
            &stgmeds[curr_index], handler->GetData(), static_cast<size_t>(handler->GetDataSize()));
...
CefRefPtr<CefDragData> DataObjectToDragData(IDataObject *data_object)
{
    ...
    do {
        ...
        for (unsigned i = 0; i < celtFetched; i++) {
            ...
            if (!(format == CF_UNICODETEXT || format == CF_TEXT || format == moz_url_format
                  || format == html_format || format == CF_HDROP)
...
CefBrowserHost::DragOperationsMask OsrStartDragging(
...
HRESULT DropTargetWin::DragEnter(
    IDataObject *data_object, DWORD key_state, POINTL cursor_position, DWORD *effect)
{
    if (!browser_handler_)
        return E_UNEXPECTED;
...
HRESULT DropTargetWin::DragOver(DWORD key_state, POINTL cursor_position, DWORD *effect)
{
    if (!browser_handler_)
        return E_UNEXPECTED;
    CefMouseEvent ev = ToMouseEvent(cursor_position, key_state, hWnd_);
    CefBrowserHost::DragOperationsMask mask = DropEffectToDragOperation(*effect);
    mask = browser_handler_->OnDragOver(ev, mask);
    *effect = DragOperationToDropEffect(mask);
    return S_OK;
}
...
HRESULT DropTargetWin::DragLeave()
{
    if (!browser_handler_)
        return E_UNEXPECTED;
    browser_handler_->OnDragLeave();
    return S_OK;
}

HRESULT DropTargetWin::Drop(
    IDataObject * /*data_object*/, DWORD key_state, POINTL cursor_position, DWORD *effect)
{
    if (!browser_handler_)
        return E_UNEXPECTED;
    CefMouseEvent ev = ToMouseEvent(cursor_position, key_state, hWnd_);
    CefBrowserHost::DragOperationsMask mask = DropEffectToDragOperation(*effect);
    mask = browser_handler_->OnDrop(ev, mask);
    *effect = DragOperationToDropEffect(mask);
    return S_OK;
}
...
{
    return DragEnumFormatEtc::CreateEnumFormatEtc(m_nNumFormats, m_pFormatEtc, ppEnumFormatEtc);
}
