    ...
    if (CefManager::GetInstance()->IsEnableOffScreenRendering()) {
        ...
        if ((m_pHandlerDelegate != nullptr) && (m_pDropTarget == nullptr)) {
            ...
            if ((pCefControl != nullptr) && (pCefControl->GetWindow() != nullptr)) {
                ...
                if (::IsWindow(hWnd)) {
                    std::shared_ptr<client::DropTargetWin> pDropTargetWin
                        = std::make_shared<client::DropTargetWin>(hWnd, this);
    ...
    CefRefPtr<CefProcessMessage> message)
{
    ...
    if (message_name == kFocusedNodeChangedMessage) {
        ...
        GlobalManager::Instance().Thread().PostTask(
            ui::kThreadUI,
            ToWeakCallback([this, browser, frame, type, bText, bEditable, nodeRect]() {
                if (m_pHandlerDelegate) {
                    m_pHandlerDelegate
                        ->OnFocusedNodeChanged(browser, frame, type, bText, bEditable, nodeRect);
                }
            }));
        return true;
        ...
        GlobalManager::Instance().Thread().PostTask(
            ui::kThreadUI, ToWeakCallback([this, fun_name, param, js_callback_id, browser, frame]() {
                if (m_pHandlerDelegate) {
                    m_pHandlerDelegate
                        ->OnExecuteCppFunc(fun_name, param, js_callback_id, browser, frame);
                }
            }));
        return true;
        ...
        GlobalManager::Instance()
            .Thread()
            .PostTask(ui::kThreadUI, ToWeakCallback([this, callback_id, param]() {
                          if (m_pHandlerDelegate) {
...
bool CefBrowserHandler::DoOnBeforePopup(
    CefRefPtr<CefBrowser> browser,
    CefRefPtr<CefFrame> frame,
    int popup_id,
    ...
    bool *no_javascript_access)
{
    ...
    if (m_pHandlerDelegate && !m_bHostWindowClosed) {
        ...
        return m_pHandlerDelegate->OnBeforePopup(
            browser,
            frame,
            popup_id,
            target_url,
...
{
    if (m_pHandlerDelegate && !m_bHostWindowClosed
        && m_pHandlerDelegate->IsCallbackExists(CefCallbackID::OnBeforePopupAborted)) {
        GlobalManager::Instance().Thread().PostTask(
            ui::kThreadUI,
            UiBind(
                &CefBrowserHandlerDelegate::OnBeforePopupAborted,
                m_pHandlerDelegate,
                browser,
                popup_id));
    }
...
void CefBrowserHandler::OnBeforeDevToolsPopup(
    CefRefPtr<CefBrowser> /*browser*/,
    CefWindowInfo & /*windowInfo*/,
    CefRefPtr<CefClient> & /*client*/,
    CefBrowserSettings & /*settings*/,
...
void CefBrowserHandler::OnAfterCreated(CefRefPtr<CefBrowser> browser)
{
    ...
    if (browser == nullptr) {
        ...
        auto hWndowHandle
            = browser->GetHost()
                  ->GetWindowHandle(); //当browser为dev tools 时，此处返回nullptr，其他情况为非nullptr值
        ...
        auto it = std::find_if(
            m_browserList.begin(),
            m_browserList.end(),
            [browser](const CefRefPtr<CefBrowser> &item) { return item->IsSame(browser); });
...
void CefBrowserHandler::OnBeforeClose(CefRefPtr<CefBrowser> browser)
{
    ...
    if (browser == nullptr) {
        ...
        std::lock_guard<std::mutex> threadGuard(m_dataMutex);
        auto it = std::find_if(
            m_browserList.begin(), m_browserList.end(), [&](const CefRefPtr<CefBrowser> &item) {
...
bool CefBrowserHandler::GetScreenPoint(
...
void CefBrowserHandler::OnPopupShow(CefRefPtr<CefBrowser> browser, bool show)
{
    if (m_pHandlerDelegate && !m_bHostWindowClosed) {
        ui::GlobalManager::Instance().Thread().PostTask(
            ui::kThreadUI,
            UiBind(&CefBrowserHandlerDelegate::OnPopupShow, m_pHandlerDelegate, browser, show));
    }
}
...
void CefBrowserHandler::OnPopupSize(CefRefPtr<CefBrowser> browser, const CefRect &rect)
{
    if (m_pHandlerDelegate && !m_bHostWindowClosed) {
        ui::GlobalManager::Instance().Thread().PostTask(
            ui::kThreadUI,
            UiBind(&CefBrowserHandlerDelegate::OnPopupSize, m_pHandlerDelegate, browser, rect));
    }
}

void CefBrowserHandler::OnPaint(
    CefRefPtr<CefBrowser> browser,
...
#if CEF_VERSION_MAJOR <= 109
//CEF 109版本
void CefBrowserHandler::OnAcceleratedPaint(
    CefRefPtr<CefBrowser> browser,
    PaintElementType type,
    const RectList &dirtyRects,
    void *shared_handle)
...
void CefBrowserHandler::GetTouchHandleSize(
    CefRefPtr<CefBrowser> browser, cef_horizontal_alignment_t orientation, CefSize &size)
{}

void CefBrowserHandler::OnTouchHandleStateChanged(
...
void CefBrowserHandler::OnImeCompositionRangeChanged(
    CefRefPtr<CefBrowser> browser, const CefRange &selected_range, const RectList &character_bounds)
{
    //此函数只有OSR模式会有回调事件
    if (m_pHandlerDelegate && !m_bHostWindowClosed) {
...
void CefBrowserHandler::OnTextSelectionChanged(
    CefRefPtr<CefBrowser> browser, const CefString &selected_text, const CefRange &selected_range)
{}

void CefBrowserHandler::OnVirtualKeyboardRequested(
    CefRefPtr<CefBrowser> browser, TextInputMode input_mode)
{}

bool CefBrowserHandler::StartDragging(
    CefRefPtr<CefBrowser> browser,
    ...
    int y)
{
    ...
    GlobalManager::Instance().Thread().PostTask(
        ui::kThreadUI,
        UiBind(&CefBrowserHandler::DoDragDrop, this, browser, drag_data, allowed_ops, x, y));
    return true;
#else
    return false;
#endif
...
void CefBrowserHandler::DoDragDrop(
    CefRefPtr<CefBrowser> browser,
    CefRefPtr<CefDragData> drag_data,
    CefRenderHandler::DragOperationsMask allowed_ops,
    int x,
    int y)
{
    ...
    CefBrowserHost::DragOperationsMask result
        = client::OsrStartDragging(drag_data, allowed_ops, x, y);
    UiPoint pt = {};
    if (spWindow != nullptr) {
...
void CefBrowserHandler::UpdateDragCursor(
...
CefBrowserHost::DragOperationsMask CefBrowserHandler::OnDragEnter(
    CefRefPtr<CefDragData> drag_data, CefMouseEvent ev, CefBrowserHost::DragOperationsMask effect)
{
    if (m_bHostWindowClosed) {
...
CefBrowserHost::DragOperationsMask CefBrowserHandler::OnDragOver(
...
CefBrowserHost::DragOperationsMask CefBrowserHandler::OnDrop(
    CefMouseEvent ev, CefBrowserHost::DragOperationsMask effect)
{
    if (m_bHostWindowClosed) {
        return DRAG_OPERATION_NONE;
...
void CefBrowserHandler::OnBeforeContextMenu(
    CefRefPtr<CefBrowser> browser,
    CefRefPtr<CefFrame> frame,
...
bool CefBrowserHandler::RunContextMenu(
    CefRefPtr<CefBrowser> browser,
    CefRefPtr<CefFrame> frame,
    CefRefPtr<CefContextMenuParams> params,
    CefRefPtr<CefMenuModel> model,
...
bool CefBrowserHandler::OnContextMenuCommand(
    CefRefPtr<CefBrowser> browser,
    CefRefPtr<CefFrame> frame,
    CefRefPtr<CefContextMenuParams> params,
    int command_id,
...
{
    if (m_pHandlerDelegate && !m_bHostWindowClosed) {
        return m_pHandlerDelegate
            ->OnContextMenuCommand(browser, frame, params, command_id, event_flags);
    }
    return false;
}

void CefBrowserHandler::OnContextMenuDismissed(
...
bool CefBrowserHandler::RunQuickMenu(
    CefRefPtr<CefBrowser> browser,
    CefRefPtr<CefFrame> frame,
...
bool CefBrowserHandler::OnQuickMenuCommand(
    CefRefPtr<CefBrowser> browser, CefRefPtr<CefFrame> frame, int command_id, EventFlags event_flags)
{
    return false;
}

void CefBrowserHandler::OnQuickMenuDismissed(CefRefPtr<CefBrowser> browser, CefRefPtr<CefFrame> frame)
{}

// CefDisplayHandler methods
void CefBrowserHandler::OnAddressChange(
    CefRefPtr<CefBrowser> browser, CefRefPtr<CefFrame> frame, const CefString &url)
{
    if (m_pHandlerDelegate && !m_bHostWindowClosed
        && m_pHandlerDelegate->IsCallbackExists(CefCallbackID::OnAddressChange)) {
        GlobalManager::Instance().Thread().PostTask(
            ui::kThreadUI,
            UiBind(
                &CefBrowserHandlerDelegate::OnAddressChange,
                m_pHandlerDelegate,
                browser,
                frame,
                url));
...
void CefBrowserHandler::OnTitleChange(CefRefPtr<CefBrowser> browser, const CefString &title)
{
    if (m_pHandlerDelegate && !m_bHostWindowClosed) {
        GlobalManager::Instance().Thread().PostTask(
            ui::kThreadUI,
            UiBind(&CefBrowserHandlerDelegate::OnTitleChange, m_pHandlerDelegate, browser, title));
    }
}

void CefBrowserHandler::OnFaviconURLChange(
    CefRefPtr<CefBrowser> browser, const std::vector<CefString> &icon_urls)
{
    if (m_pHandlerDelegate && !m_bHostWindowClosed
        && m_pHandlerDelegate->IsCallbackExists(CefCallbackID::OnFaviconURLChange)) {
        GlobalManager::Instance().Thread().PostTask(
            ui::kThreadUI,
            UiBind(
                &CefBrowserHandlerDelegate::OnFaviconURLChange,
                m_pHandlerDelegate,
                browser,
                icon_urls));
    }
...
{
    if (m_pHandlerDelegate && !m_bHostWindowClosed
        && m_pHandlerDelegate->IsCallbackExists(CefCallbackID::OnFullscreenModeChange)) {
        GlobalManager::Instance().Thread().PostTask(
            ui::kThreadUI,
            UiBind(
                &CefBrowserHandlerDelegate::OnFullscreenModeChange,
                m_pHandlerDelegate,
                browser,
                fullscreen));
    }
...
{
    if (m_pHandlerDelegate && !m_bHostWindowClosed
        && m_pHandlerDelegate->IsCallbackExists(CefCallbackID::OnStatusMessage)) {
        GlobalManager::Instance().Thread().PostTask(
            ui::kThreadUI,
            UiBind(&CefBrowserHandlerDelegate::OnStatusMessage, m_pHandlerDelegate, browser, value));
    }
}

bool CefBrowserHandler::OnConsoleMessage(
    CefRefPtr<CefBrowser> /*browser*/,
...
{
    if (m_pHandlerDelegate && !m_bHostWindowClosed
        && m_pHandlerDelegate->IsCallbackExists(CefCallbackID::OnLoadingProgressChange)) {
        GlobalManager::Instance().Thread().PostTask(
            ui::kThreadUI,
            UiBind(
                &CefBrowserHandlerDelegate::OnLoadingProgressChange,
                m_pHandlerDelegate,
                browser,
                progress));
    }
...
bool CefBrowserHandler::OnCursorChange(
    CefRefPtr<CefBrowser> browser,
    ...
    const CefCursorInfo &custom_cursor_info)
{
    ...
    if (!CefManager::GetInstance()->IsEnableOffScreenRendering()) {
        ...
        if (m_pHandlerDelegate && !m_bHostWindowClosed) {
            GlobalManager::Instance().Thread().PostTask(
                ui::kThreadUI,
                UiBind(&CefBrowserHandlerDelegate::OnCursorChange, m_pHandlerDelegate, type));
        }
#else
        //非SDL模式下，需要主动设置光标
...
void CefBrowserHandler::OnMediaAccessChange(
    CefRefPtr<CefBrowser> browser, bool has_video_access, bool has_audio_access)
{
    if (m_pHandlerDelegate && !m_bHostWindowClosed
        && m_pHandlerDelegate->IsCallbackExists(CefCallbackID::OnMediaAccessChange)) {
        GlobalManager::Instance().Thread().PostTask(
            ui::kThreadUI,
            UiBind(
                &CefBrowserHandlerDelegate::OnMediaAccessChange,
                m_pHandlerDelegate,
                browser,
                has_video_access,
                has_audio_access));
...
bool CefBrowserHandler::OnDragEnter(
    CefRefPtr<CefBrowser> browser,
...
void CefBrowserHandler::OnDraggableRegionsChanged(
    CefRefPtr<CefBrowser> browser,
    CefRefPtr<CefFrame> frame,
    const std::vector<CefDraggableRegion> &regions)
{
    if (m_pHandlerDelegate && !m_bHostWindowClosed
        && m_pHandlerDelegate->IsCallbackExists(CefCallbackID::OnDraggableRegionsChanged)) {
        GlobalManager::Instance().Thread().PostTask(
            ui::kThreadUI,
            UiBind(
                &CefBrowserHandlerDelegate::OnDraggableRegionsChanged,
                m_pHandlerDelegate,
                browser,
                frame,
                regions));
...
void CefBrowserHandler::OnLoadingStateChange(
    CefRefPtr<CefBrowser> browser, bool isLoading, bool canGoBack, bool canGoForward)
{
    if (m_pHandlerDelegate && !m_bHostWindowClosed
        && m_pHandlerDelegate->IsCallbackExists(CefCallbackID::OnLoadingStateChange)) {
        GlobalManager::Instance().Thread().PostTask(
            ui::kThreadUI,
            UiBind(
                &CefBrowserHandlerDelegate::OnLoadingStateChange,
                m_pHandlerDelegate,
                browser,
                isLoading,
                canGoBack,
...
void CefBrowserHandler::OnLoadStart(
    CefRefPtr<CefBrowser> browser, CefRefPtr<CefFrame> frame, TransitionType transition_type)
{
    if (m_pHandlerDelegate && !m_bHostWindowClosed
        && m_pHandlerDelegate->IsCallbackExists(CefCallbackID::OnLoadStart)) {
        GlobalManager::Instance().Thread().PostTask(
            ui::kThreadUI,
            UiBind(
                &CefBrowserHandlerDelegate::OnLoadStart,
                m_pHandlerDelegate,
                browser,
                frame,
                transition_type));
...
void CefBrowserHandler::OnLoadEnd(
    CefRefPtr<CefBrowser> browser, CefRefPtr<CefFrame> frame, int httpStatusCode)
{
    if (m_pHandlerDelegate && !m_bHostWindowClosed
        && m_pHandlerDelegate->IsCallbackExists(CefCallbackID::OnLoadEnd)) {
        GlobalManager::Instance().Thread().PostTask(
            ui::kThreadUI,
            UiBind(
                &CefBrowserHandlerDelegate::OnLoadEnd,
                m_pHandlerDelegate,
                browser,
                frame,
                httpStatusCode));
...
void CefBrowserHandler::OnLoadError(
    CefRefPtr<CefBrowser> browser,
    CefRefPtr<CefFrame> frame,
    ErrorCode errorCode,
...
{
    if (m_pHandlerDelegate && !m_bHostWindowClosed
        && m_pHandlerDelegate->IsCallbackExists(CefCallbackID::OnLoadError)) {
        GlobalManager::Instance().Thread().PostTask(
            ui::kThreadUI,
            UiBind(
                &CefBrowserHandlerDelegate::OnLoadError,
                m_pHandlerDelegate,
                browser,
                frame,
                errorCode,
...
bool CefBrowserHandler::OnJSDialog(
    CefRefPtr<CefBrowser> /*browser*/,
...
bool CefBrowserHandler::OnBeforeUnloadDialog(
    CefRefPtr<CefBrowser> browser,
    const CefString &message_text,
    bool is_reload,
    CefRefPtr<CefJSDialogCallback> callback)
...
bool CefBrowserHandler::OnPreKeyEvent(
    CefRefPtr<CefBrowser> browser,
...
bool CefBrowserHandler::OnKeyEvent(
...
bool CefBrowserHandler::OnBeforeBrowse(
    CefRefPtr<CefBrowser> browser,
    CefRefPtr<CefFrame> frame,
...
bool CefBrowserHandler::GetAuthCredentials(
    CefRefPtr<CefBrowser> browser,
    const CefString &origin_url,
    bool isProxy,
...
bool CefBrowserHandler::OnCertificateError(
    CefRefPtr<CefBrowser> browser,
...
bool CefBrowserHandler::OnSelectClientCertificate(
    CefRefPtr<CefBrowser> browser,
    bool isProxy,
    const CefString &host,
    int port,
...
#if CEF_VERSION_MAJOR > 109
//CEF 高版本
bool CefBrowserHandler::OnRenderProcessUnresponsive(
    CefRefPtr<CefBrowser> browser, CefRefPtr<CefUnresponsiveProcessCallback> callback)
{
    return false;
}
...
void CefBrowserHandler::OnRenderProcessTerminated(
    CefRefPtr<CefBrowser> browser,
...
{
    if (m_pHandlerDelegate && !m_bHostWindowClosed
        && m_pHandlerDelegate->IsCallbackExists(CefCallbackID::OnRenderProcessTerminated)) {
        GlobalManager::Instance().Thread().PostTask(
            ui::kThreadUI,
            UiBind(
                &CefBrowserHandlerDelegate::OnRenderProcessTerminated,
                m_pHandlerDelegate,
                browser,
                status,
                error_code,
...
{
    if (m_pHandlerDelegate && !m_bHostWindowClosed
        && m_pHandlerDelegate->IsCallbackExists(CefCallbackID::OnDocumentAvailableInMainFrame)) {
        GlobalManager::Instance().Thread().PostTask(
            ui::kThreadUI,
            UiBind(
                &CefBrowserHandlerDelegate::OnDocumentAvailableInMainFrame,
                m_pHandlerDelegate,
                browser));
    }
}

CefRefPtr<CefCookieAccessFilter> CefBrowserHandler::GetCookieAccessFilter(
...
CefRefPtr<CefResourceHandler> CefBrowserHandler::GetResourceHandler(
    CefRefPtr<CefBrowser> browser, CefRefPtr<CefFrame> frame, CefRefPtr<CefRequest> request)
{
    return nullptr;
}

void CefBrowserHandler::OnResourceRedirect(
    CefRefPtr<CefBrowser> browser,
    CefRefPtr<CefFrame> frame,
    CefRefPtr<CefRequest> request,
...
bool CefBrowserHandler::OnResourceResponse(
    CefRefPtr<CefBrowser> browser,
    CefRefPtr<CefFrame> frame,
    CefRefPtr<CefRequest> request,
    CefRefPtr<CefResponse> response)
...
CefRefPtr<CefResponseFilter> CefBrowserHandler::GetResourceResponseFilter(
    CefRefPtr<CefBrowser> browser,
    CefRefPtr<CefFrame> frame,
    CefRefPtr<CefRequest> request,
    CefRefPtr<CefResponse> response)
...
void CefBrowserHandler::OnResourceLoadComplete(
    CefRefPtr<CefBrowser> browser,
    CefRefPtr<CefFrame> frame,
    CefRefPtr<CefRequest> request,
    CefRefPtr<CefResponse> response,
...
{
    if (m_pHandlerDelegate && !m_bHostWindowClosed) {
        m_pHandlerDelegate->OnResourceLoadComplete(
            browser, frame, request, response, status, received_content_length);
    }
}

void CefBrowserHandler::OnProtocolExecution(
    CefRefPtr<CefBrowser> browser,
    CefRefPtr<CefFrame> frame,
    CefRefPtr<CefRequest> request,
...
bool CefBrowserHandler::CanSendCookie(
    CefRefPtr<CefBrowser> browser,
    CefRefPtr<CefFrame> frame,
    CefRefPtr<CefRequest> request,
...
bool CefBrowserHandler::CanSaveCookie(
    CefRefPtr<CefBrowser> browser,
    CefRefPtr<CefFrame> frame,
    CefRefPtr<CefRequest> request,
    CefRefPtr<CefResponse> response,
...
bool CefBrowserHandler::CanDownload(
    CefRefPtr<CefBrowser> browser, const CefString &url, const CefString &request_method)
{
    if (m_pHandlerDelegate && !m_bHostWindowClosed) {
...
#if CEF_VERSION_MAJOR <= 109
//CEF 109版本
void CefBrowserHandler::OnBeforeDownload(
    CefRefPtr<CefBrowser> browser,
...
#else
//CEF 高版本
bool CefBrowserHandler::OnBeforeDownload(
    CefRefPtr<CefBrowser> browser,
    CefRefPtr<CefDownloadItem> download_item,
    const CefString &suggested_name,
    CefRefPtr<CefBeforeDownloadCallback> callback)
...
#if CEF_VERSION_MAJOR <= 109
//CEF 109版本
bool CefBrowserHandler::OnFileDialog(
    CefRefPtr<CefBrowser> browser,
    FileDialogMode mode,
...
{
    if (m_pHandlerDelegate && !m_bHostWindowClosed) {
        return m_pHandlerDelegate->OnFileDialog(
            browser,
            mode,
            title,
            default_file_path,
...
{
    if (m_pHandlerDelegate && !m_bHostWindowClosed && (browser != nullptr) && (m_browser != nullptr)
        && browser->IsSame(m_browser)) {
        GlobalManager::Instance().Thread().PostTask(
            ui::kThreadUI, UiBind(&CefBrowserHandlerDelegate::OnGotFocus, m_pHandlerDelegate));
    }
}
