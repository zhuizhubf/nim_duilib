    ...
    const CefString &process_type, CefRefPtr<CefCommandLine> command_line)
{
    ...
    if (process_type.empty()) {
        ...
        DString currentVideoDriver = StringUtil::MakeLowerString(
            MessageLoop_SDL::GetCurrentVideoDriverName());
        ...
        if (IsWaylandEnvironment() && (currentVideoDriver == _T("x11"))) {
            ...
            if (!dpiFactor.empty()) {
                command_line->AppendSwitchWithValue(
                    "force-device-scale-factor", CefString(dpiFactor.c_str()));
            }
        }
        ...
        //添加额外的参数（应用层配置）
        const std::vector<std::pair<DString, DString>> &cefSwitchWithValues
            = CefManager::GetInstance()->GetSwitchWithValues();
