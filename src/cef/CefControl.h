...
public:
    ...
    */
    bool RegisterCppFunc(
        const DString &function_name, CppFunction function, bool global_function = false);
    ...
    */
    bool CallJSFunction(
        ...
        CallJsFunctionCallback callback,
...
public:
    ...
    */
    void AttachBeforeContextMenu(const OnBeforeContextMenuEvent &callback)
    {
        m_pfnBeforeContextMenu = callback;
    }

    /** 绑定一个回调函数用于监听选择了哪个右键菜单（回调函数的调用线程：CEF的UI线程）
    * @param [in] callback 一个回调函数，参考 OnContextMenuCommandEvent 声明
    */
    void AttachContextMenuCommand(const OnContextMenuCommandEvent &callback)
    {
        m_pfnContextMenuCommand = callback;
    }

    /** 绑定一个回调函数用于监听右键菜单消失（回调函数的调用线程：CEF的UI线程）
    * @param [in] callback 一个回调函数，参考 OnContextMenuDismissedEvent 声明
    */
    void AttachContextMenuDismissed(const OnContextMenuDismissedEvent &callback)
    {
        m_pfnContextMenuDismissed = callback;
    }
    ...
    */
    void AttachFaviconURLChange(OnFaviconURLChangeEvent callback)
    {
        m_pfnFaviconURLChange = callback;
    }

    /** 绑定一个回调函数用于监听页面的全屏状态改变（回调函数的调用线程：主进程的UI线程）
    * @param [in] callback 一个回调函数，参考 OnFullscreenModeChangeEvent 声明
    */
    void AttachFullscreenModeChange(OnFullscreenModeChangeEvent callback)
    {
        m_pfnFullscreenModeChange = callback;
    }
    ...
    */
    void AttachLoadingProgressChange(OnLoadingProgressChangeEvent callback)
    {
        m_pfnLoadingProgressChange = callback;
    }

    /** 绑定一个回调函数用于监听页面的多媒体访问情况改变（回调函数的调用线程：主进程的UI线程）
    * @param [in] callback 一个回调函数，参考 OnMediaAccessChangeEvent 声明
    */
    void AttachMediaAccessChange(OnMediaAccessChangeEvent callback)
    {
        m_pfnMediaAccessChange = callback;
    }
    ...
    */
    void AttachDraggableRegionsChanged(OnDraggableRegionsChangedEvent callback)
    {
        m_pfnDraggableRegionsChanged = callback;
    }
    ...
    */
    void AttachBeforePopupAborted(const OnBeforePopupAbortedEvent &callback)
    {
        m_pfnBeforePopupAborted = callback;
    }

    /** 绑定一个回调函数用于监听页面资源加载事件（回调函数的调用线程：CEF的UI线程）
    * @param [in] callback 一个回调函数，参考 OnBeforeResourceLoadEvent 声明
    */
    void AttachBeforeResourceLoad(const OnBeforeResourceLoadEvent &callback)
    {
        m_pfnBeforeResourceLoad = callback;
    }

    /** 绑定一个回调函数用于监听页面资源加载重定向事件（回调函数的调用线程：CEF的UI线程）
    * @param [in] callback 一个回调函数，参考 OnResourceRedirectEvent 声明
    */
    void AttachResourceRedirect(const OnResourceRedirectEvent &callback)
    {
        m_pfnResourceRedirect = callback;
    }

    /** 绑定一个回调函数用于监听页面资源加载收到回应事件（回调函数的调用线程：CEF的UI线程）
    * @param [in] callback 一个回调函数，参考 OnResourceResponseEvent 声明
    */
    void AttachResourceResponse(const OnResourceResponseEvent &callback)
    {
        m_pfnResourceResponse = callback;
    }

    /** 绑定一个回调函数用于监听页面资源加载完成事件（回调函数的调用线程：CEF的UI线程）
    * @param [in] callback 一个回调函数，参考 OnResourceLoadCompleteEvent 声明
    */
    void AttachResourceLoadComplete(const OnResourceLoadCompleteEvent &callback)
    {
        m_pfnResourceLoadComplete = callback;
    }

    /** 绑定一个回调函数用于监听一个未知的 Protocol（回调函数的调用线程：CEF的UI线程）
    * @param[in] callback 一个回调函数，参考 OnProtocolExecutionEvent 声明
    */
    void AttachProtocolExecution(const OnProtocolExecutionEvent &callback)
    {
        m_pfnProtocolExecution = callback;
    }
    ...
    */
    ...
    {
        m_pfnLoadingStateChange = callback;
    }
    ...
    */
    ...
    {
        m_pfnDevToolVisibleChange = callback;
    };
    ...
    */
    void AttachBeforeDownload(const OnBeforeDownloadEvent &callback)
    {
        m_pfnBeforeDownload = callback;
    }

    /** 绑定一个回调函数用于监听下载过程中任务的状态改变通知（回调函数的调用线程：CEF的UI线程）
    * @param[in] callback 一个回调函数，参考 OnDownloadUpdatedEvent 声明
    */
    void AttachDownloadUpdated(const OnDownloadUpdatedEvent &callback)
    {
        m_pfnDownloadUpdated = callback;
    }
    ...
    */
    void AttachDocumentAvailableInMainFrame(const OnDocumentAvailableInMainFrameEvent &callback)
    {
        m_pfnDocumentAvailableInMainFrame = callback;
    }

    /** 绑定一个回调函数用于监听网站图标下载完成的通知（回调函数的调用线程：主线程的UI线程）
    * @param[in] callback 一个回调函数，参考 OnDownloadFavIconFinishedEvent 声明
    */
    void AttachDownloadFavIconFinished(const OnDownloadFavIconFinishedEvent &callback)
    {
        m_pfnDownloadFavIconFinished = callback;
    }
...
protected:
    ...
    */
    ...
    virtual void OnAddressChange(
        CefRefPtr<CefBrowser> browser, CefRefPtr<CefFrame> frame, const CefString &url) override;
    ...
    virtual bool OnKeyEvent(
        CefRefPtr<CefBrowser> browser, const CefKeyEvent &event, CefEventHandle os_event) override;
