...
public:
    ...
    virtual void AddBezier(
        ...
        int32_t y4) override;
    virtual void AddBezier(
        float x1, float y1, float x2, float y2, float x3, float y3, float x4, float y4) override;
...
public:
    ...
    public:
        TImpl()
            : m_path(Gdiplus::FillModeAlternate)
        {}
...
private:
    ...
    static int CALLBACK
    EnumFontCallback(const LOGFONTW *lpelfe, const TEXTMETRICW *, DWORD fontType, LPARAM lParam);
