...
public:
    ...
    virtual int32_t SetRoundClip(
        const UiRect &rcItem, float rx, float ry, bool bIntersect = true) override;
    ...
    virtual void DrawLine(
        const UiPointF &pt1, const UiPointF &pt2, UiColor penColor, float fWidth) override;
    virtual void DrawLine(const UiPointF &pt1, const UiPointF &pt2, IPen *pen) override;
    virtual void DrawRect(
        const UiRectF &rc, UiColor penColor, float fWidth, bool bLineInRect = false) override;
    ...
    virtual void DrawRoundRect(
        const UiRectF &rc, float rx, float ry, UiColor penColor, float fWidth) override;
    virtual void DrawRoundRect(const UiRectF &rc, float rx, float ry, IPen *pen) override;
    virtual void FillRoundRect(
        const UiRectF &rc, float rx, float ry, UiColor dwColor, uint8_t uFade = 255) override;
    virtual void FillRoundRect(
        const UiRectF &rc,
        float rx,
        float ry,
    ...
    virtual void DrawArc(
        ...
        const UiRect *gradientRect = nullptr) override;
    virtual void DrawCircle(
        const UiPointF &centerPt, float radius, UiColor penColor, float fWidth) override;
    virtual void DrawCircle(const UiPointF &centerPt, float radius, IPen *pen) override;
    virtual void FillCircle(
        const UiPointF &centerPt, float radius, UiColor dwColor, uint8_t uFade = 255) override;
    ...
    virtual void FillPath(
        ...
        UiColor dwColor,
        UiColor dwColor2,
        int8_t nColor2Direction) override;

    virtual UiRect MeasureString(
        const DString &strText, const MeasureStringParam &measureParam) override;
    ...
    virtual bool IsDrawRichTextCacheEqual(
        const DrawRichTextCache &first, const DrawRichTextCache &second) const override;
    ...
    virtual void RestoreAlpha(
        const UiRect &rcDirty, const UiPadding &rcShadowPadding, uint8_t alpha) override;
    virtual void RestoreAlpha(
        const UiRect &rcDirty, const UiPadding &rcShadowPadding = UiPadding()) override;
    ...
    virtual bool WritePixels(
        void *srcPixels, size_t srcPixelsLen, const UiRect &rc, const UiRect &rcPaint) override;
