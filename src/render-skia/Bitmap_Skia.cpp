    ...
    BitmapAlphaType alphaType)
{
    ...
    if (ImageUtil::NeedResizeImage(fImageSizeScale)) {
        ...
        if (pPixelBits == nullptr) {
            ...
            m_pSkBitmap->setInfo(
                SkImageInfo::Make(
                    nNewWidth, nNewHeight, kN32_SkColorType, static_cast<SkAlphaType>(alphaType)));
            ...
            bool bRet = ImageUtil::ResizeImageData(
    ...
    uint32_t nWidth, uint32_t nHeight, const void *pPixelBits, BitmapAlphaType alphaType)
{
    ...
    m_pSkBitmap->reset();
    m_pSkBitmap->setInfo(
        SkImageInfo::Make(nWidth, nHeight, kN32_SkColorType, static_cast<SkAlphaType>(alphaType)));
    m_pSkBitmap->allocPixels();
...
IBitmap *Bitmap_Skia::Clone()
{
    ...
    if (!pBitmap->Init(
            ...
            pPixelBits,
            1.0f,
            static_cast<BitmapAlphaType>(m_pSkBitmap->info().alphaType()))) {
        delete pBitmap;
        pBitmap = nullptr;
    }
    return pBitmap;
