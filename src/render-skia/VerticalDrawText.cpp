...
UTF32String VerticalDrawText::GetDrawStringUTF32(const DString &strText, bool bSingleLineMode) const
{
...
bool VerticalDrawText::NeedRotateForVertical(DUTF32Char ch) const
{
    // 拉丁字母、数字、通用符号：需要旋转
    if (
        // 大写字母
        ...
        (ch >= 0x30 && ch <= 0x39) ||
        // 基础标点符号（!@#$%^&*等）
        (ch >= 0x21 && ch <= 0x2F) || (ch >= 0x3A && ch <= 0x40) || (ch >= 0x5B && ch <= 0x60)
        || (ch >= 0x7B && ch <= 0x7E) ||
        // 半角标点
        (ch >= 0xFF01 && ch <= 0xFF5E) ||
        // 其他符号（如箭头、数学符号等）
        (ch >= 0x2190 && ch <= 0x21FF) || (ch >= 0x2200 && ch <= 0x22FF)) {
        return true; // 需要旋转90度
    }
...
bool VerticalDrawText::CalculateTextCharBounds(
    ...
    std::vector<TVerticalChar> &charRects) const
{
    ...
    for (DUTF32Char ch : textUTF32) {
        ...
        if (ch == L'\n') {
            ...
            verticalChar.bRotate90 = false;
            SkScalar fTextWidth = DrawSkiaText::MeasureTextChar(
                *pSkFont,
                ch,
                ...
                pFont,
                true);
            if ((fTextWidth < fMinCharSize) || (verticalChar.bounds.width() < fMinCharSize)
                || (verticalChar.bounds.height() < fMinCharSize)) {
                //空格或者不可见字符(按小写字母确定显示区域)
                if (fTextWidth > fMinCharSize) {
                    int32_t nTextWidth = (int32_t) (ceilf(fTextWidth) + 0.5f);
                    verticalChar.bounds = SkRect::MakeIWH(nTextWidth, nTextWidth);
                } else {
                    ch = DrawSkiaText::GetMeasureDefaultChar();
                    fTextWidth = DrawSkiaText::MeasureTextChar(
                        *pSkFont,
                        ch,
                        &verticalChar.bounds, //斜体字时，这个宽度包含了外延的宽度
                        skPaint,
                ...
                //用字体的实际高度，作为字的高度
                verticalChar.size = SkSize::Make(
                    std::max(fTextWidth, verticalChar.bounds.width()),
                    verticalChar.bounds.height()); // 用实际高度
            } else if (bUseFontHeight) {
                //用字体高度作为字的高度，所有字都等高
                verticalChar.size = SkSize::Make(
                    std::max(fTextWidth, verticalChar.bounds.width()), (SkScalar) fFontHeight);
            } else {
                //用字体的实际高度，作为字的高度
                verticalChar.size = SkSize::Make(
                    std::max(fTextWidth, verticalChar.bounds.width()),
...
SkRect VerticalDrawText::CalculateVerticalTextBounds(
...
float VerticalDrawText::CalculateDefaultCharWidth(
    const IFont *pFont, const SkFont *pSkFont, const SkPaint *skPaint) const
{
    ...
    SkRect bounds;
    SkScalar fCharWidth = DrawSkiaText::MeasureTextChar(
        *pSkFont,
        ch,
...
UiRect VerticalDrawText::MeasureString(const DString &strText, const MeasureStringParam &measureParam)
{
    ...
    if (!CalculateTextCharBounds(
            textUTF32,
            measureParam.pFont,
            pSkFont,
            &skPaint,
            measureParam.bUseFontHeight,
            (float) fFontHeight,
            measureParam.bRotate90ForAscii,
            charRects)) {
        return UiRect();
    }
    ASSERT(charRects.size() == textUTF32.size());
    ...
    SkRect skTextBounds = CalculateVerticalTextBounds(
        charRects,
        measureParam.rectSize,
        bSingleLineMode,
        measureParam.fSpacingMul,
        measureParam.fSpacingAdd,
...
void VerticalDrawText::DrawString(const DString &strText, const DrawStringParam &drawParam)
{
    ...
    if (!CalculateTextCharBounds(
            ...
            pSkFont,
            &skPaint,
            drawParam.bUseFontHeight,
            (float) fFontHeight,
            drawParam.bRotate90ForAscii,
            charRects)) {
        return;
    }
    ASSERT(charRects.size() == textUTF32.size());
    ...
    SkRect skTextBounds = CalculateVerticalTextBounds(
        charRects,
        drawParam.textRect.Height(),
        bSingleLineMode,
        fSpacingMul,
        fSpacingAdd,
        fWordVerticalSpacing,
        fDefaultCharWidth,
        fFontHeight,
        &columnRows,
        &columnWidths,
    ...
    for (size_t charIndex = 0; charIndex < drawCharCount; ++charIndex) {
        ...
        if (bNeedEllipsis && (charIndex < (drawCharCount - 1))) { // 是否绘制"..."的业务逻辑
            ...
            if (nextCharPos.nColumnIndex == charPos.nColumnIndex) {
                ...
                if ((yPos > (SkScalar) rcSkDest.fBottom)
                    && (bSingleLineMode
                        || (charPos.xPos - fColumnWidth) < (SkScalar) rcSkDest.fLeft)) {
                    //下个字符无法显示了
            ...
            if (bNeedDrawEllipsis) {
                ...
                skCanvas->translate(
                    charPos.xPos,
                    charPos.bRotate90 ? charPos.yPos : charPos.yPos - charPos.chHeight / 2);
                skCanvas->rotate(90);
        ...
        if (charPos.bRotate90) {
            ...
            DrawSkiaText::DrawSimpleText(
                skCanvas, charPos.ch, 0, 0, *pSkFont, skPaint, drawParam.pFont);
            skCanvas->restoreToCount(saveCount);
        } else {
            DrawSkiaText::DrawSimpleText(
                skCanvas, charPos.ch, charPos.xPos, charPos.yPos, *pSkFont, skPaint, drawParam.pFont);
        }
    }

    //绘制下划线/删除线
    if ((drawParam.pFont->IsUnderline() || drawParam.pFont->IsStrikeOut())
        && !columnWidths.empty()) {
        ...
        for (size_t nColumnIndex = 0; nColumnIndex < textColumnInfo.size(); ++nColumnIndex) {
            ...
            if (textColumn.bEndFlag) {
                x_scalar = (SkIntToScalar(textColumn.ptStart.x) + SkIntToScalar(textColumn.ptEnd.x))
                           / 2;
            }
            ...
            if (textColumn.bEndFlag) {
                height = (SkScalar) (textColumn.ptEnd.y - textColumn.ptStart.y
                                     + textColumn.chEndHeight * 4 / 5);
                if (textColumn.bStartRotate90) {
            ...
            if (drawParam.pFont->IsStrikeOut()) {
                ...
                const SkRect r = SkRect::MakeLTRB(
                    x_scalar + xOffset, top, x_scalar + xOffset + width, top + height);
