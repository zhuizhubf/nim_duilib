...
public:
    ...
    */
    ...
    /** 获取字体个数
    */
    uint32_t GetFontCont() const { return (uint32_t) m_fontFamilies.size(); }
...
bool FontMgr_Skia::HasFontName(const DString &fontName) const
{
    ...
    for (int nIndex = 0; nIndex < nCountFamilies; ++nIndex) {
        SkString
            fontFamilyName; //字体名称，该名称为与系统语言相同的字体名称，比如中文版时返回"微软雅黑"，英文版时返回"Microsoft YaHei"，所以很可能匹配不到设置的字体
        m_impl->m_pSkFontMgr->getFamilyName((int) nIndex, &fontFamilyName);
    ...
    if (!bFound) {
        ...
        for (int nIndex = 0; nIndex < nCountFamilies; ++nIndex) {
            ...
            if (!fontFamilyName.isEmpty()) {
                auto skTypeFace
                    = m_impl->m_pSkFontMgr
                          ->legacyMakeTypeface(fontFamilyName.c_str(), SkFontStyle::Normal());
...
SkFont *FontMgr_Skia::CreateSkFont(const UiFont &fontInfo)
{
    ...
    if (!m_impl->m_defaultFontName.empty() && (m_impl->m_defaultFontName != fontInfo.m_fontName)
        && HasFontName(m_impl->m_defaultFontName)) {
