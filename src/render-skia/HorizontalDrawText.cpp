...
bool HorizontalDrawText::CalculateTextCharBounds(
        ...
        if (ch == L'\n') {
            ...
            SkScalar fTextWidth = DrawSkiaText::MeasureTextChar(
                *pSkFont,
                ...
                pFont,
                true);
            if ((fTextWidth < fMinCharSize) || (horizontalChar.bounds.width() < fMinCharSize)
                || (horizontalChar.bounds.height() < fMinCharSize)) {
            ...
            //用字体高度作为字的高度，所有字都等高
            horizontalChar.size = SkSize::Make(
                std::max(fTextWidth, horizontalChar.bounds.width()), (SkScalar) fFontHeight);
            charRects.push_back(horizontalChar);
        }
    }
...
SkRect HorizontalDrawText::CalculateHorizontalTextBounds(
...
float HorizontalDrawText::CalculateDefaultCharWidth(
...
UiRect HorizontalDrawText::MeasureString(
    const DString &strText, const MeasureStringParam &measureParam)
{
    ...
    if (!CalculateTextCharBounds(
            textUTF32, measureParam.pFont, pSkFont, &skPaint, (float) fFontHeight, charRects)) {
        return UiRect();
    }
    ...
    SkRect skTextBounds = CalculateHorizontalTextBounds(
        charRects,
...
void HorizontalDrawText::DrawString(const DString &strText, const DrawStringParam &drawParam)
{
    ...
    std::vector<THorizontalChar> charRects;
    if (!CalculateTextCharBounds(
            textUTF32, drawParam.pFont, pSkFont, &skPaint, (float) fFontHeight, charRects)) {
        return;
    }
    ASSERT(charRects.size() == textUTF32.size());
    if (charRects.size() != textUTF32.size()) {
    ...
    for (size_t nRowIndex = 0; nRowIndex < nRowCount; ++nRowIndex) {
        ...
        for (size_t nColIndex = 0; nColIndex < nColCount; ++nColIndex) {
            ...
            // 水平位置
            charPos.xPos = xPos
                           + (horizontalChar.bounds.left() < 0 ? -horizontalChar.bounds.left() : 0);
        ...
        if (bNeedEllipsis && (charIndex < (drawCharCount - 1))) { // 是否绘制"..."的业务逻辑
            ...
            if (nextCharPos.nRowIndex == charPos.nRowIndex) {
                ...
                if ((xPos > (SkScalar) rcSkDest.fRight)
                    && (bSingleLineMode
                        || (charPos.yPos + fRowHeight) > (SkScalar) rcSkDest.fBottom)) {
                    //下个字符无法显示了
        ...
        for (size_t nRowIndex = 0; nRowIndex < textRowInfo.size(); ++nRowIndex) {
            ...
            if (drawParam.pFont->IsUnderline()) {
                ...
                SkRect r = SkRect::MakeLTRB(
                    x_scalar,
                    y + text_size * kUnderlineOffset,
                    x_scalar + width,
                    y + (text_size * (kUnderlineOffset + (thickness_factor * kLineThicknessFactor))));
                skCanvas->drawRect(r, skPaint);
            }
        }
    }
