...
bool SkiaTextShaper::ResolveGlyph(const IFont* pFont, uint32_t unicodeChar, TextGlyphInfo& glyph, bool bUseDefaultCharWhenFailed)
{
    ...
    if (glyphId == 0) {
        ...
        if (pFallbackFontMgr != nullptr) {
            ...
            IFont* pFallbackFont = pFallbackFontMgr->CreateFallbackFont(pFont, nResolvedChar, &nFallbackGlyphId);
    ...
    const float fAdvance = DrawSkiaText::MeasureTextChar(*pFontHandle, nResolvedChar, &bounds,
                                                         m_pRender->m_pSkPaint.get(), pFont,
...
void SkiaTextShaper::DrawGlyph(const TextGlyphInfo& glyph, float x, float y, UiColor textColor, uint8_t uFade)
{
    ...
    skPaint.setColor(SkColorSetARGB(nAlpha, textColor.GetRed(), textColor.GetGreen(), textColor.GetBlue()));
    const SkPoint& ptOrg = m_pRender->GetPointOrg();
    DrawSkiaText::DrawSimpleText(pCanvas, glyph.m_unicodeChar,
                                 x + ptOrg.x(), y + ptOrg.y(),
                                 *pFontHandle, skPaint, glyph.m_pFont);
}
