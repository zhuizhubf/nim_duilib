    ...
    if (SUCCEEDED(hr) && (pfd != nullptr)) {
        ...
        if (!defaultLocation.IsEmpty()) {
            ...
            hr = SHCreateItemFromParsingName(
                defaultPath.c_str(), nullptr, IID_IShellItem, reinterpret_cast<void **>(&psi));
    ...
    const FilePath &defaultLocation)
{
    ...
    if (bOpenFileDialog) {
        ...
        hr = ::CoCreateInstance(
            CLSID_FileSaveDialog, nullptr, CLSCTX_INPROC_SERVER, IID_PPV_ARGS(&pfd));
    }
    //过滤类型固定为Unicode字符串
    struct FileTypeW
    {
    ...
    for (const FileType &fileType : fileTypes) {
        ...
        fileTypesW.push_back(
            {StringConvert::TToWString(name), StringConvert::TToWString(fileType.szExt)});
    }
    ...
    if (SUCCEEDED(hr) && (pfd != nullptr)) {
        ...
        if (!fileTypesW.empty()) {
            ...
            for (const FileTypeW &fileType : fileTypesW) {
                filterSpecs.push_back(
                    {(LPCWSTR) fileType.szName.c_str(), (LPCWSTR) fileType.szExt.c_str()});
            }
        ...
        if ((nFileTypeIndex >= 0) && (nFileTypeIndex < (int32_t) fileTypesW.size())) {
            hr = pfd->SetFileTypeIndex(
                (UINT) nFileTypeIndex + 1); //SetFileTypeIndex的下标是从1开始的
            ASSERT(SUCCEEDED(hr));
        }
        if (!defaultExt.empty()) {
