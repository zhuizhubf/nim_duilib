...
void WindowImplBase::PreInitWindow()
{
    ...
    if (pControl) {
        pControl->AttachBubbledEvent(
            ui::kEventMouseDoubleClick,
            UiBind(&WindowImplBase::OnTitleBarDoubleClick, this, std::placeholders::_1),
            0);
    }
#endif
}
...
void WindowImplBase::OnSelectLanguage(Control *pBtnSelectLanguage)
{
    ...
    ui::Menu *menu = new ui::Menu(
        this, pBtnSelectLanguage); //需要设置父窗口，否在菜单弹出的时候，程序状态栏编程非激活状态
...
void WindowImplBase::OnSelectTheme(Control *pBtnSelectTheme)
{
    ...
    //显示选择主题菜单
    ui::Menu *menu = new ui::Menu(
        this, pBtnSelectTheme); //需要设置父窗口，否在菜单弹出的时候，程序状态栏编程非激活状态
    WindowPtr spMenuWnd(menu);
    //设置菜单xml所在的目录
    menu->SetSkinFolder(DString(DUILIB_PUBLIC_RES_DIR) + _T("/menu/"));
...
LRESULT WindowImplBase::OnSizeMsg(
...
Control *WindowImplBase::GetBtnWindowByName(
    const DString &newCtrlName, const DString &oldCtrlName) const
{
