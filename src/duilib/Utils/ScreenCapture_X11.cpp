...
static void ExtractRGBA(
    XImage *ximage, uint32_t pixel, uint8_t &r, uint8_t &g, uint8_t &b, uint8_t &a)
{
    r = g = b = 0xFF;
    a = 0xFF; // 默认完全不透明
    ...
    if (ximage->depth >= 24) {
        // 动态提取RGB分量（根据显示的颜色掩码）
        r = (pixel & ximage->red_mask)
            >> ((ximage->red_mask != 0) ? __builtin_ctz(ximage->red_mask) : 0);
        g = (pixel & ximage->green_mask)
            >> ((ximage->green_mask != 0) ? __builtin_ctz(ximage->green_mask) : 0);
        b = (pixel & ximage->blue_mask)
            >> ((ximage->blue_mask != 0) ? __builtin_ctz(ximage->blue_mask) : 0);

        // 处理Alpha通道（不依赖alpha_mask，通过深度和RGB掩码总位数推断）
        if (ximage->depth == 32) {
            // 计算RGB三个掩码的总位数
            int rgbTotalBits = MaskBitCount(ximage->red_mask) + MaskBitCount(ximage->green_mask)
                               + MaskBitCount(ximage->blue_mask);

            // 如果RGB总位数 < 32，说明存在Alpha通道（剩余位数为Alpha）
            if (rgbTotalBits < 32) {
                // 查找Alpha通道的位置（未被RGB占用的高位）
                uint32_t alphaMask = ~(ximage->red_mask | ximage->green_mask | ximage->blue_mask)
                                     & 0xFFFFFFFF;
                if (alphaMask != 0) {
...
// 返回值:
//   成功返回true，失败返回false
static bool CaptureScreenBitmap(
    ::Display *display,
    ::Window targetWindow,
