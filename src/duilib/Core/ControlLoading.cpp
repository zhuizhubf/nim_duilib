...
bool ControlLoading::StartLoading(int32_t nIntervalMs, int32_t nMaxCount)
{
    ...
    if (m_pLoadingBox == nullptr) {
        ...
        Box *pLoadingBox = GlobalManager::Instance().CreateBox(
            m_pControl->GetWindow(), FilePath(m_pLoadingAttribute->m_sXmlPath.c_str()));
        ...
        if (!m_pLoadingAttribute->m_sAnimationControlName.empty()) {
            m_pAnimationControl = m_pControl->GetWindow()->FindSubControlByName(
                m_pLoadingBox.get(), m_pLoadingAttribute->m_sAnimationControlName.c_str());
        }
    }
    ...
    GlobalManager::Instance().Timer().AddTimer(
        m_loadingFlag.GetWeakFlag(), UiBind(&ControlLoading::Loading, this), m_nIntervalMs);
