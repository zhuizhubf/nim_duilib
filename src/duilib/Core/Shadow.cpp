...
public:
    ...
    virtual UiEstSize EstimateSize(UiSize szAvailable) override
    {
        ...
        for (auto pControl : m_items) {
            ...
            if ((pControl->GetFixedWidth().IsAuto()) || (pControl->GetFixedHeight().IsAuto())) {
    ...
    virtual void PaintBkImage(IRender *pRender) override
    {
        ...
        if ((m_pShadow == nullptr) || !m_pShadow->IsShadowAttached()) {
            ...
            if (m_pShadow != nullptr) {
                ...
                int32_t nShadowBorderSize
                    = m_pShadow
                          ->GetShadowBorderSize(); //边框大小(以XmlRoot Box的矩形为中心线，中心线两侧各一个像素)
                DString shadowBorderColor
                    = m_pShadow->GetShadowBorderColor(); //边框颜色（和Win11默认的窗口边框颜色接近）
        ...
        if (IsShadowAttached() && IsSystemShadowEnabled(nShadowType)) {
            ...
            if (m_pWindow->IsUseSystemCaption()) {
                m_pWindow->NativeWnd()->SetSystemShadowType(
                    NativeWindowShadowType::kShadowSystemDoNotRound);
                //启用系统阴影时，必须清除RGN，否则显示不正确(由调用方负责处理)
                m_pWindow->NativeWnd()->ClearWindowRgn(true);
            } else {
                m_pWindow->NativeWnd()->SetSystemShadowType(
                    NativeWindowShadowType::kShadowSystemDisabled);
            }
#else
            m_pWindow->NativeWnd()->SetSystemShadowType(
                NativeWindowShadowType::kShadowSystemDisabled);
    ...
    Shadow *pShadowObj)
{
    ...
    if ((pShadowObj != nullptr) && !pShadowObj->GetShadowBorderColor().empty()) {
        svgReplaceColors = StringUtil::Printf(
            _T("svg_replace_colors='#B5B5B5|%s'"), pShadowObj->GetShadowBorderColor().c_str());
    }
    if (nShadowType == ShadowType::kShadowBig) {
        bRet = true;
        szBorderRound = UiSize(0, 0);
        rcShadowCorner = UiPadding(30, 30, 34, 36);
        shadowImage = StringUtil::Printf(
            _T("file='%s/shadow/shadow_big.svg' window_shadow_mode='true' corner='%d,%d,%d,%d' %s"),
        ...
        rcShadowCorner = UiPadding(30, 30, 34, 36);
        shadowImage = StringUtil::Printf(
            _T("file='%s/shadow/shadow_big_round.svg' window_shadow_mode='true' ")
            _T("corner='%d,%d,%d,%d' %s"),
            DUILIB_PUBLIC_RES_DIR,
            rcShadowCorner.left + szBorderRound.cx,
            rcShadowCorner.top + szBorderRound.cx,
        ...
        shadowImage = StringUtil::Printf(
            _T("file='%s/shadow/shadow_small.svg' window_shadow_mode='true' corner='%d,%d,%d,%d' ")
            _T("%s"),
            DUILIB_PUBLIC_RES_DIR,
            rcShadowCorner.left + szBorderRound.cx,
            rcShadowCorner.top + szBorderRound.cx,
        ...
        rcShadowCorner = UiPadding(24, 24, 28, 30);
        shadowImage = StringUtil::Printf(
            _T("file='%s/shadow/shadow_small_round.svg' window_shadow_mode='true' ")
            _T("corner='%d,%d,%d,%d' %s"),
            DUILIB_PUBLIC_RES_DIR,
            rcShadowCorner.left + szBorderRound.cx,
            rcShadowCorner.top + szBorderRound.cx,
        ...
        rcShadowCorner = UiPadding(24, 24, 28, 30);
        shadowImage = StringUtil::Printf(
            _T("file='%s/shadow/shadow_menu.svg' window_shadow_mode='true' corner='%d,%d,%d,%d' ")
            _T("%s"),
            DUILIB_PUBLIC_RES_DIR,
            rcShadowCorner.left + szBorderRound.cx,
            rcShadowCorner.top + szBorderRound.cx,
        ...
        rcShadowCorner = UiPadding(24, 24, 28, 30);
        shadowImage = StringUtil::Printf(
            _T("file='%s/shadow/shadow_menu_round.svg' window_shadow_mode='true' ")
            _T("corner='%d,%d,%d,%d' %s"),
            DUILIB_PUBLIC_RES_DIR,
            rcShadowCorner.left + szBorderRound.cx,
            rcShadowCorner.top + szBorderRound.cx,
    ...
    if ((pShadowObj != nullptr)
        ...
        if (nShadowBorderSize > 0) {
            ...
            rcShadowCorner = UiPadding(
                nShadowBorderSize,
                nShadowBorderSize,
                nShadowBorderSize,
                nShadowBorderSize); //边线宽度
        } else {
...
{
    ASSERT(
        (rcShadowCorner.left >= 0) && (rcShadowCorner.top >= 0) && (rcShadowCorner.right >= 0)
        && (rcShadowCorner.bottom >= 0));
    if ((rcShadowCorner.left >= 0) && (rcShadowCorner.top >= 0) && (rcShadowCorner.right >= 0)
        && (rcShadowCorner.bottom >= 0)) {
...
void Shadow::SetWindowPosSnap(bool bLeftSnap, bool bRightSnap, bool bTopSnap, bool bBottomSnap)
{
    if (IsEnableShadowSnap() && IsShadowAttached() && !GetShadowImage().empty()) {
        if ((m_bLeftSnap != bLeftSnap) || (m_bTopSnap != bTopSnap) || (m_bRightSnap != bRightSnap)
        ...
        if ((m_bLeftSnap != 0) || (m_bTopSnap != 0) || (m_bRightSnap != 0) || (m_bBottomSnap != 0)) {
            m_bLeftSnap = 0;
            m_bTopSnap = 0;
            m_bRightSnap = 0;
            m_bBottomSnap = 0;
...
void Shadow::CheckMouseClickOnShadow(EventType eventType, const UiPoint &pt)
{
    ...
    if ((rcShadowCorner.left <= 1) && (rcShadowCorner.top <= 1) && (rcShadowCorner.right <= 1)
        && (rcShadowCorner.bottom <= 1)) {
        //当前无阴影
        return;
    }
