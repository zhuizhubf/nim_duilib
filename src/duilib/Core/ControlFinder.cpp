...
Control *ControlFinder::FindControl(const UiPoint &pt) const
{
    ...
    if (m_pRoot != nullptr) {
        ...
        return m_pRoot->FindControl(
            FindControlFromPoint, &ptLocal, UIFIND_VISIBLE | UIFIND_HITTEST | UIFIND_TOP_FIRST, pt);
    }
    return nullptr;
}
...
Control *ControlFinder::FindToolTipControl(const UiPoint &pt) const
{
    ...
    if (m_pRoot != nullptr) {
        UiPoint ptLocal = pt;
        return m_pRoot->FindControl(
            FindControlFromPoint,
            &ptLocal,
            UIFIND_VISIBLE | UIFIND_HITTEST | UIFIND_TOP_FIRST | UIFIND_TOOLTIP,
            pt);
    }
    return nullptr;
}
    ...
    if (m_pRoot != nullptr) {
        if (pt != nullptr) {
            ...
            pControl = m_pRoot->FindControl(
                FindContextMenuControl,
                &ptLocal,
                UIFIND_VISIBLE | UIFIND_ENABLED | UIFIND_HITTEST | UIFIND_TOP_FIRST,
                ptLocal);
        } else {
            pControl = m_pRoot->FindControl(
                FindContextMenuControl, nullptr, UIFIND_VISIBLE | UIFIND_ENABLED | UIFIND_TOP_FIRST);
        }
    }
    return pControl;
}
...
Box *ControlFinder::FindDroppableBox(const UiPoint &pt, uint8_t nDropInId) const
{
    ...
    if (m_pRoot != nullptr) {
        pControl = m_pRoot->FindControl(
            FindControlFromDroppableBox,
            (void *) (size_t) nDropInId,
            UIFIND_VISIBLE | UIFIND_HITTEST | UIFIND_TOP_FIRST | UIFIND_DRAG_DROP,
            pt);
    }
    if (pControl != nullptr) {
...
Control *ControlFinder::FindSubControlByName(Control *pParent, const DString &strName) const
{
    ...
    if (pParent != nullptr) {
        pFindControl
            = pParent->FindControl(FindControlFromName, (void *) strName.c_str(), UIFIND_ALL);
        if (pFindControl != nullptr) {
            return pFindControl;
        }
        if (pOldRoot != nullptr) {
            //控件全屏状态，使用窗口原来的根容器查找
            pFindControl
                = pOldRoot->FindControl(FindControlFromName, (void *) strName.c_str(), UIFIND_ALL);
            if (pFindControl != nullptr) {
                return pFindControl;
            }
        }
    ...
    if (iter != m_controlNameMap.end()) {
        ...
        for (const ControlPtr &spControl : controlList) {
            if (pAncestor != nullptr) {
                if ((spControl != nullptr) && (pAncestor->GetWindow() == spControl->GetWindow())
...
void ControlFinder::AddControl(Control *pControl)
{
    ...
    if (iter != m_controlNameMap.end()) {
        ...
        const size_t nMaxControlCount
            = (size_t) 10; //名字重复的控件，最多只保存10个，避免过多导致性能降低
