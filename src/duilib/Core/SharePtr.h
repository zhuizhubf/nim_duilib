...
private:
    ...
    int32_t GetRefCount() const { return m_nRefCnt.load(std::memory_order_relaxed); }
#endif
 ...
 */
template<typename T>
static inline T *SafeAddRef(T *obj)
{
    if (obj) {
        obj->AddRef();
    }
 ...
 */
template<typename T>
static inline void SafeRelease(T *obj)
{
    if (obj) {
        obj->Release();
    }
...
public:
    NVRefCount()
        : m_nRefCnt(1)
    {}
    ~NVRefCount() { ASSERT(m_nRefCnt.load(std::memory_order_relaxed) == 1); }
...
public:
    ...
    constexpr SharePtr()
        : m_ptr(nullptr)
    {}
    constexpr SharePtr(std::nullptr_t)
        : m_ptr(nullptr)
    {}
     ...
     */
    SharePtr(const SharePtr<T> &that)
        : m_ptr(SafeAddRef(that.get()))
    {}
    template<typename U, typename = typename std::enable_if<std::is_convertible<U *, T *>::value>::type>
    SharePtr(const SharePtr<U> &that)
        : m_ptr(SafeAddRef(that.get()))
    {}
     ...
     */
    SharePtr(SharePtr<T> &&that)
        : m_ptr(that.release())
    {}
    template<typename U, typename = typename std::enable_if<std::is_convertible<U *, T *>::value>::type>
    SharePtr(SharePtr<U> &&that)
        : m_ptr(that.release())
    {}
    ...
    SharePtr<T> &operator=(std::nullptr_t)
    {
        this->reset();
        return *this;
    }
...
template<typename T>
inline void swap(SharePtr<T> &a, SharePtr<T> &b) /*noexcept*/
{
    a.swap(b);
}

template<typename T, typename U>
inline bool operator==(const SharePtr<T> &a, const SharePtr<U> &b)
{
    return a.get() == b.get();
}
template<typename T>
inline bool operator==(const SharePtr<T> &a, std::nullptr_t) /*noexcept*/
{
    return !a;
}
template<typename T>
inline bool operator==(std::nullptr_t, const SharePtr<T> &b) /*noexcept*/
{
    return !b;
}
 ...
 */
template<typename T>
SharePtr<T> ref_share_ptr(T *obj)
{
    return SharePtr<T>(SafeAddRef(obj));
}

template<typename T>
SharePtr<T> ref_share_ptr(const T *obj)
{
