...
bool ControlDropTargetUtils::IsFilteredFileTypes(
    const DString &fileTypes, const std::vector<DString> &fileList)
{
...
void ControlDropTargetUtils::RemoveUnsupportedFiles(
    std::vector<DString> &fileList, const DString &fileTypes)
{
    if (fileTypes.empty() || fileList.empty()) {
        return;
    }
