...
public:
    ...
    bool Expired() const { return m_weak_flag.expired(); }

    const std::weak_ptr<WeakFlag> &GetFlag() const { return m_weak_flag; }

    const T &GetFunc() const { return m_func; }
...
public:
    ...
    auto ToWeakCallback(const CallbackType &closure) -> WeakCallback<CallbackType>
    {
...
auto UiBind(F &&f, Args &&...args) -> decltype(std::bind(f, args...))
{
...
template<
    ...
    class... Args,
    class = typename std::enable_if<!std::is_member_function_pointer<F>::value>::type>
auto UiBind(C *p, F &&f, Args &&...args) -> WeakCallback<decltype(std::bind(f, args...))>
{
    std::weak_ptr<WeakFlag> weak_flag = p->GetWeakFlag();
    auto bind_obj = std::bind(f, args...);
    static_assert(
        std::is_base_of<SupportWeakCallback, C>::value, "SupportWeakCallback should be base of C");
