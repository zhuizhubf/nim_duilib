...
static ::Window FindWindowBelow(Display *display, ::Window window, int rootX, int rootY)
{
    ...
    if (targetIndex != -1) {
        ...
        for (int i = targetIndex - 1; i >= 0; i--) {
            ...
            XWindowAttributes attrs;
            if (XGetWindowAttributes(display, candidate, &attrs)
...
static int ActivateX11Window(::Window window, int force)
{
    ...
    if (supports_netwm) {
        ...
        XSendEvent(
            display, root, False, SubstructureRedirectMask | SubstructureNotifyMask, (XEvent *) &ev);
    }
