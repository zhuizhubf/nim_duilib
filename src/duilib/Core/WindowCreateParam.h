...
enum WindowStyle : uint32_t {
    ...
    kWS_CAPTION = 0x00C00000L,
    /* kWS_BORDER | kWS_DLGFRAME */ //窗口具有标题栏，(包含 kWS_BORDER 样式) 。
    ...
    kWS_OVERLAPPEDWINDOW
    = (kWS_OVERLAPPED | kWS_CAPTION | kWS_SYSMENU | kWS_THICKFRAME | kWS_MINIMIZEBOX
       | kWS_MAXIMIZEBOX),
    //弹出窗口
...
enum WindowExStyle : uint32_t {
    kWS_EX_TOPMOST
    = 0x00000008L, //窗口应放置在所有非最顶部窗口的上方，并且应保持在窗口上方，即使窗口已停用也是如此。
    kWS_EX_ACCEPTFILES = 0x00000010L,    //窗口接受拖放文件
    kWS_EX_TRANSPARENT = 0x00000020L,    //该窗口显示为透明
    kWS_EX_TOOLTIP_WINDOW = 0x00000040L, //该窗口旨在用作Tooltip窗口（仅当SDL时使用）
    kWS_EX_TOOLWINDOW
    = 0x00000080L, //该窗口旨在用作浮动工具栏。 工具窗口具有短于普通标题栏的标题栏和使用较小的字体绘制的窗口标题。 工具窗口不会显示在任务栏中，也不会显示在用户按 Alt+TAB 时显示的对话框中。 如果工具窗口具有系统菜单，则其图标不会显示在标题栏上。
    kWS_EX_LAYERED = 0x00080000L, //该窗口是一个分层窗口
    kWS_EX_NOACTIVATE
    = 0x08000000L //用户单击时，使用此样式创建的顶级窗口不会成为前台窗口。 当用户最小化或关闭前台窗口时，系统不会将此窗口带到前台。
};

/** 窗口的位置和大小默认值标志
*/
enum WindowDefaultSize { kCW_USEDEFAULT = ((int32_t) 0x80000000) };
...
public:
    ...
    */
    WindowCreateParam(
        const DString &windowTitle, bool bCenterWindow, const DString &windowId = _T(""));
