...
DString ListCtrl::GetType() const
{
    return DUI_CTR_LISTCTRL;
}
        ...
        if ((m_pReportView != nullptr) && (m_pHeaderCtrl != nullptr)) {
            ...
            if (nAvailableWidth >= (int32_t) stretchWidthMap.size()) {
                ...
                for (const auto &iter : stretchWidthMap) {
                    ...
                    int32_t nColunmWidth = iter.second.GetStretchPercentValue() * nAvailableWidth
                                           / nStretchBase;
        ...
        if (m_pReportView != nullptr) {
            ...
            if ((itemCount > 0) && (nStartIndex < (itemCount - 1))) {
                for (size_t index = nStartIndex + 1; index < itemCount; ++index) {
                    ListCtrlItem *pCheckItem = dynamic_cast<ListCtrlItem *>(
                        m_pReportView->GetItemAt(index));
...
ListCtrlIconViewItem *ListCtrl::GetNextDisplayIconItem(ListCtrlIconViewItem *pItem) const
{
    ...
    if (pItem == nullptr) {
        ...
        if (m_pIconView != nullptr) {
            ...
            if ((itemCount > 0) && (nStartIndex < (itemCount - 1))) {
                for (size_t index = nStartIndex + 1; index < itemCount; ++index) {
                    ListCtrlIconViewItem *pCheckItem = dynamic_cast<ListCtrlIconViewItem *>(
                        m_pIconView->GetItemAt(index));
                    if ((pCheckItem != nullptr) && pCheckItem->IsVisible()) {
                        pNextItem = pCheckItem;
                        break;
                    }
...
ListCtrlListViewItem *ListCtrl::GetNextDisplayListItem(ListCtrlListViewItem *pItem) const
{
    ...
    if (pItem == nullptr) {
        ...
        if (m_pListView != nullptr) {
            ...
            if ((itemCount > 0) && (nStartIndex < (itemCount - 1))) {
                for (size_t index = nStartIndex + 1; index < itemCount; ++index) {
                    ListCtrlListViewItem *pCheckItem = dynamic_cast<ListCtrlListViewItem *>(
                        m_pListView->GetItemAt(index));
                    if ((pCheckItem != nullptr) && pCheckItem->IsVisible()) {
                        pNextItem = pCheckItem;
                        break;
                    }
...
void ListCtrl::OnColumnSorted(size_t nColumnId, bool bSortedUp)
{
    ...
    m_pData
        ->SortDataItems(nColumnId, GetColumnIndex(nColumnId), bSortedUp, nSortFlag, nullptr, nullptr);
    Refresh();
}
...
bool ListCtrl::SortDataItemsById(
    size_t columnId,
    bool bSortedUp,
    uint8_t nSortFlag,
    ListCtrlDataCompareFunc pfnCompareFunc,
    void *pUserData)
{
    ...
    return m_pData
        ->SortDataItems(columnId, columnIndex, bSortedUp, nSortFlag, pfnCompareFunc, pUserData);
}
...
bool ListCtrl::SetDataItemCount(size_t itemCount)
{
    ...
    if (bRet) {
        ...
        if (bChanged) {
            ...
            SendEvent(
                kEventDataItemCountChanged, (WPARAM) nNewDataItemCount, (LPARAM) nOldDataItemCount);
        }
    }
    return bRet;
}
...
bool ListCtrl::SetSubItemData(
    size_t itemIndex, size_t columnIndex, const ListCtrlSubItemData &subItemData)
{
    return SetSubItemDataById(itemIndex, GetColumnId(columnIndex), subItemData);
}

bool ListCtrl::SetSubItemDataById(
...
bool ListCtrl::GetSubItemData(
    size_t itemIndex, size_t columnIndex, ListCtrlSubItemData &subItemData) const
{
    return GetSubItemDataById(itemIndex, GetColumnId(columnIndex), subItemData);
}

bool ListCtrl::GetSubItemDataById(
...
void ListCtrl::OnItemEnterEditMode(
...
bool ListCtrl::IsValidItemEditState(const ListCtrlEditParam &editParam) const
{
    ...
    if (editParam.listCtrlType == ListCtrlType::Icon) {
        ...
        ASSERT(
            (pItem != nullptr) && pItem->IsVisible() && pItem->IsSelected() && pItem->IsFocused());
        if ((pItem == nullptr) || !pItem->IsVisible() || !pItem->IsSelected()
            || !pItem->IsFocused()) {
            return false;
        }
        ASSERT(m_pIconView != nullptr);
...
void ListCtrl::OnItemEditMode(ListCtrlEditParam editParam)
{
    ...
    if (enterEditParam.bCancelled || !IsValidItemEditState(editParam)
        || !IsValidItemEditParam(editParam)) {
        //状态已经失效, 或者用户取消编辑
        ClearEditEvents();
        RestoreItemFocus();
        LeaveEditMode();
    ...
    m_pRichEdit->AttachEvent(
        kEventKeyDown,
        [this, RestoreItemFocus](const EventArgs &msg) {
...
{
    if ((msg.eventType == kEventMouseWheel) || (msg.eventType == kEventMouseButtonDown)
        || (msg.eventType == kEventMouseButtonUp) || (msg.eventType == kEventMouseDoubleClick)
        || (msg.eventType == kEventMouseRButtonDown) || (msg.eventType == kEventMouseRButtonUp)
        || (msg.eventType == kEventMouseRDoubleClick) || (msg.eventType == kEventContextMenu)) {
