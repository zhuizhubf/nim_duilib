...
DString RichEdit2::GetType() const
{
...
bool RichEdit2::FindRichText(const FindTextParam &findParam, TextCharRange &chrgText) const
{
    ...
    bool bRet = m_pTextData->FindRichText(
        findParam.bMatchCase,
        findParam.bMatchWholeWord,
        findParam.bFindDown,
        findParam.chrg.cpMin,
        findParam.chrg.cpMax,
        findText,
        nFoundStartChar,
        nFoundEndChar);
...
void RichEdit2::OnScrollOffsetChanged(
...
void RichEdit2::Paint(IRender *pRender, const UiRect &rcPaint)
{
    ...
    if (!IsPasswordMode()) {
        ...
        if (spDrawRichTextCache != nullptr) {
            //校验缓存是否失效
            if (!pRender
                     ->IsValidDrawRichTextCache(rcDrawText, richTextDataList, spDrawRichTextCache)) {
        ...
        if (spDrawRichTextCache != nullptr) {
            ...
            pRender->DrawRichTextCacheData(
                spDrawRichTextCache,
                rcDrawText,
                szScrollOffset,
                m_pTextData->GetTextRowXOffset(),
                GetAlpha());
            ...
            IRenderFactory *pRenderFactory = GlobalManager::Instance().GetRenderFactory();
            ASSERT(pRenderFactory != nullptr);
            pRender->CreateDrawRichTextCache(
                rcDrawText, szScrollOffset, pRenderFactory, richTextDataList, spDrawRichTextCache);
            if (spDrawRichTextCache != nullptr) {
                ASSERT(pRender->IsValidDrawRichTextCache(
                    rcDrawText, richTextDataList, spDrawRichTextCache));
                //通过缓存绘制
                rcDrawText.Offset(0, m_pTextData->GetTextRectOfssetY());
                pRender->DrawRichTextCacheData(
                    spDrawRichTextCache,
                ...
                //当前后端不支持绘制缓存，直接绘制文本
                rcDrawText.Offset(0, m_pTextData->GetTextRectOfssetY());
                pRender->DrawRichText(
                    rcDrawText, szScrollOffset, pRenderFactory, richTextDataList, GetAlpha());
            }
        }
    } else {
        //密码模式，不使用绘制缓存
...
void RichEdit2::ShowCaret(bool fShow)
{
    ...
    if (m_bTextInputMode) {
        ...
        //设置输入区域
        int32_t nCursorOffset
            = xWidth
              + Dpi().GetScaleInt(1); //输入法的候选框与光标当前位置的距离（水平方向）, 避免遮盖光标
        Window *pWindow = GetWindow();
        if (pWindow != nullptr) {
...
void RichEdit2::PaintCaret(IRender *pRender, const UiRect & /*rcPaint*/)
{
    ...
    if (m_bIsCaretVisible) {
        ...
        if (rcCaret.Intersect(rcDrawText)) {
            ...
            pRender->DrawLine(
                UiPointF(xPos + 1, yPos),
                UiPointF(xPos + 1, yPos + yHeight),
                dwClrColor,
                (float) xWidth);
        }
    }
...
bool RichEdit2::GetRichTextForDraw(
...
bool RichEdit2::OnKeyDown(const EventArgs &msg)
{
    ...
    if (OnCtrlArrowKeyDownScrollView(msg)) {
        ...
        (msg.vkCode == kVK_RETURN) || (msg.vkCode == kVK_TAB) || (msg.vkCode == kVK_DELETE)
        || (msg.vkCode == kVK_BACK)) {
...
bool RichEdit2::OnCtrlArrowKeyDownScrollView(const EventArgs &msg)
{
    ...
    if (IsKeyDown(msg, ModifierKey::kControl)) {
        if ((msg.vkCode == kVK_DOWN) || (msg.vkCode == kVK_UP) || (msg.vkCode == kVK_NEXT)
            || (msg.vkCode == kVK_PRIOR) || (msg.vkCode == kVK_HOME) || (msg.vkCode == kVK_END)) {
...
bool RichEdit2::OnArrowKeyDown(const EventArgs &msg)
{
    ...
    if ((msg.vkCode == kVK_LEFT) || (msg.vkCode == kVK_RIGHT) || (msg.vkCode == kVK_DOWN)
        || (msg.vkCode == kVK_UP) || (msg.vkCode == kVK_NEXT) || (msg.vkCode == kVK_PRIOR)
    ...
    if (IsNumberOnly() && !IsReadOnly() && IsEnabled()
...
{
    if ((m_bMouseDown || m_bRMouseDown) && (pSender != nullptr) && (m_pMouseSender == pSender)
        && pSender->IsMouseFocused()) {
        UiSize64 scrollPos = GetScrollPos();
...
uint32_t RichEdit2::GetNextZoomPercent(uint32_t nOldZoomPercent, bool bZoomIn) const
{
    ...
    for (size_t nPercentIndex = 0; nPercentIndex < nPercentCount; ++nPercentIndex) {
        if ((zoomPercentList[nPercentIndex] > nZoomPercent)
            || (nPercentIndex == (nPercentCount - 1))) {
            if (nPercentIndex <= 1) {
...
void RichEdit2::OnCheckScrollView()
{
    ...
    if (bScrollView) {
        ...
        //启动定时器
        m_scrollViewFlag.Cancel();
        GlobalManager::Instance().Timer().AddTimer(
            m_scrollViewFlag.GetWeakFlag(),
...
void RichEdit2::OnInputChar(const EventArgs &msg)
{
    ...
    if (msg.eventType == ui::kEventKeyDown) {
        ...
        ASSERT(
            (msg.vkCode == kVK_RETURN) || (msg.vkCode == kVK_TAB) || (msg.vkCode == kVK_DELETE)
            || (msg.vkCode == kVK_BACK));
        if ((msg.vkCode != kVK_RETURN) && (msg.vkCode != kVK_TAB) && (msg.vkCode != kVK_DELETE)
            && (msg.vkCode != kVK_BACK)) {
            return;
        }
    } else {
    ...
    if (msg.vkCode == kVK_DELETE) {
        ...
        if (nSelEndChar > nSelStartChar) {
            ...
            text.clear();
            bool bMatchWord
                = IsKeyDown(msg, ModifierKey::kControl); //Ctrl + Delete键，删除光标后面的单词
            if (bMatchWord) {
        ...
        if (nSelEndChar > nSelStartChar) {
            ...
            text.clear();
            bool bMatchWord
                = IsKeyDown(msg, ModifierKey::kControl); //Ctrl + Backspace键，删除光标前面的单词
            if (bMatchWord) {
                nSelStartChar = m_pTextData->GetPrevValidWordIndex(nSelStartChar);
            } else {
                nSelStartChar = m_pTextData->GetPrevValidCharIndex(nSelStartChar);
    ...
    //是否检测数字模式
    bool bCheckNumberOnly = IsNumberOnly()
                            && ((GetMinNumber() != INT_MIN) || (GetMaxNumber() != INT_MAX));
    DStringW oldText;
    if (bCheckNumberOnly) {
    ...
    if (IsPasswordMode() && !IsShowPassword() && IsFlashPasswordChar()) {
        if (nNewSelChar == GetTextLength()) {
            ...
            GlobalManager::Instance()
                .Timer()
                .AddTimer(m_falshPasswordFlag.GetWeakFlag(), closure, 1500);
        }
    }
