...
DString CircleProgress::GetType() const
{
...
void CircleProgress::PaintStateImages(IRender *pRender)
{
    ...
    if (pIndicatorImageInfo != nullptr) {
        outer.Inflate(
            -1 * pIndicatorImageInfo->GetWidth() / 2, -1 * pIndicatorImageInfo->GetWidth() / 2);
    } else {
    ...
    if (pIndicatorImageInfo != nullptr) {
        ...
        if ((spMatrix != nullptr) && ((nMax - nMin) != 0)) {
            ...
            spMatrix->RotateAt(
                angle, (float) center.x - ptScrollOffset.x, (float) center.y - ptScrollOffset.y);
        }
        ...
        DString imageModify = StringUtil::Printf(
