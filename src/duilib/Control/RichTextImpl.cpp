...
void RichTextImpl::PaintText(IRender *pRender)
{
    ...
    if (!m_textData.empty()) {
        ...
        if (m_spDrawRichTextCache == nullptr) {
            bool bRet = pRender->CreateDrawRichTextCache(
                textRect, szScrollOffset, pRenderFactory, richTextData, m_spDrawRichTextCache);
            if (!bRet) {
                m_spDrawRichTextCache.reset();
            }
        }
...
bool RichTextImpl::ParseTextSlice(
    ...
    std::vector<RichTextDataEx> &textData) const
{
    ...
    currentTextData.m_textView
        = std::wstring_view(textSlice.m_text.c_str(), textSlice.m_text.size());
    ...
    if (textSlice.m_fontInfo.m_fontSize > 0) {
        currentTextData.m_pFontInfo->m_fontSize = m_pOwner->Dpi().GetScaleInt(
            textSlice.m_fontInfo.m_fontSize); //字体大小，需要DPI缩放
    }
