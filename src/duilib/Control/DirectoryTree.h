...
public:
    ...
    */
    struct PathInfo
    {
        ...
        uint32_t m_nIconID
            = 0; //关联图标ID（GlobalManager::Instance().Icon().AddIcon的返回值，图标需要实现类添加到管理器）
    ...
    */
    typedef std::function<
        void(ui::TreeNode *pTreeNode, const std::vector<ui::DirectoryTree::DiskInfo> &diskInfoList)>
        ShowMyComputerContentsEvent;
...
public:
    ...
    */
    TreeNode *ShowVirtualDirectoryNode(
        VirtualDirectoryType type, const DString &displayName, bool bDisplayNameIsID = false);
    ...
    */
    TreeNode *ShowAllDiskNodes(
        const DString &computerName, const DString &fileSystemName, bool bDisplayNameIsID = false);
