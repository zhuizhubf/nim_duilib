...
void TextDrawer::UpdateTextDrawProps(
...
UiRect TextDrawer::MeasureString(
    IRender *pRender,
    ...
    Control *pOwner)
{
    ...
    if (bRichText && (pOwner != nullptr)) {
        ...
        UpdateTextDrawProps(
            measureParam.uFormat, measureParam.fSpacingMul, measureParam.fSpacingAdd, fontId);

        m_pRichText->SetWindow(
            pOwner->GetWindow()); //绘制过程中和解析RichText的时候，需要关联窗口（需要Dpi缩放等操作）
