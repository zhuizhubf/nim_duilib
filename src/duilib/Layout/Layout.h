...
public:
    ...
    */
    virtual bool IsVLayout() const
    {
        ...
        return (type == LayoutType::VLayout) || (type == LayoutType::VFlowLayout)
               || (type == LayoutType::VTileLayout) || (type == LayoutType::VirtualVLayout)
    ...
    */
    ...
    {
        LayoutType type = GetLayoutType();
        return (type == LayoutType::HLayout) || (type == LayoutType::HFlowLayout)
               || (type == LayoutType::HTileLayout) || (type == LayoutType::VirtualHLayout)
    ...
    */
    ...
    {
        LayoutType type = GetLayoutType();
        return (type == LayoutType::VTileLayout) || (type == LayoutType::HTileLayout)
               || (type == LayoutType::VirtualVTileLayout)
               || (type == LayoutType::VirtualHTileLayout);
    }
    ...
    virtual UiSize64 ArrangeChildren(
        const std::vector<Control *> &items, UiRect rc, bool bEstimateOnly = false);
...
private:
     ...
     * @return 返回控件最终的大小信息（宽度和高度）
     */
    static UiSize64 SetFloatPosInternal(
        Control *pControl, const UiRect &rcContainer, bool bEstimateOnly);
