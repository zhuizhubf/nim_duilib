    ...
    ui::attr::control::Id id, const DString &strName, const DString &strValue2)
{
    ...
    case ui::attr::control::kFadeSwitchType: {
        SetFadeSwitchTypeByString(strValue);
        break;
    }
    case ui::attr::control::kFadeSwitchFrameIntervalMs: {
        SetFadeSwitchFrameIntervalMillSeconds(StringUtil::StringToInt32(strValue));
        break;
    }
    case ui::attr::control::kFadeSwitchTotalMs: {
        SetFadeSwitchTotalMillSeconds(StringUtil::StringToInt32(strValue));
        break;
    }
    case ui::attr::control::kFadeSwitchEasingFunction: {
...
bool TabBox::StartSwitchItemAnimationFadeInOutX(Control *pNewItemControl, Control *pOldItemControl)
{
    ...
    AnimationPlayCallback playCallback =
        [pTabBox, pNewItem, pOldItem, nMaxValue, bRightToLeft](int32_t nNewValue) {
            ...
            const int32_t nBoxItemOffset = static_cast<int32_t>(
                (int64_t) nBoxWidth * nNewValue / nMaxValue);
            if (bRightToLeft) {
...
bool TabBox::StartSwitchItemAnimationFadeInOut(Control *pNewItemControl, Control *pOldItemControl)
{
    ...
    AnimationPlayCallback playCallback =
        [pTabBox, pNewItem, pOldItem, nMaxValue, nNewItemAlpha, nOldItemAlpha](int32_t nNewValue) {
            if ((pTabBox == nullptr) || (pNewItem == nullptr) || (pOldItem == nullptr)) {
                return;
            }
            //确保两个页面都显示
            ...
            //计算当前的Alpha值，并更新
            uint8_t nCurrentNewItemAlpha = static_cast<uint8_t>(
                nNewItemAlpha * nNewValue / nMaxValue);
            uint8_t nCurrentOldItemAlpha = static_cast<uint8_t>(
                nOldItemAlpha * (nMaxValue - nNewValue) / nMaxValue);
    ...
    AnimationCompleteCallback compelteCallback =
        [pTabBox, pNewItem, pOldItem, nNewItemAlpha, nOldItemAlpha]() {
            //动画结束
            if ((pTabBox != nullptr) && (pOldItem != nullptr)) {
                size_t itemIndex = pTabBox->GetItemIndex(pOldItem.get());
                if (itemIndex < pTabBox->GetItemCount()) {
