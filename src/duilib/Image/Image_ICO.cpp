...
bool Image_ICO::LoadImageFromMemory(
    ...
    int32_t nFrameDelayMs)
{
    ...
    for (int i = 0; i < nFrameCount; ++i) {
        ...
        if ((icoData.m_imageWidth > nImageSize) || (icoData.m_imageHeight > nImageSize)) {
            ...
            bInitOk = pFrameData->m_pBitmap->Init(
                icoData.m_imageWidth,
                icoData.m_imageHeight,
                icoData.m_imageData.data(),
                fNewImageSizeScale);
        } else {
            bInitOk = pFrameData->m_pBitmap->Init(
                icoData.m_imageWidth,
