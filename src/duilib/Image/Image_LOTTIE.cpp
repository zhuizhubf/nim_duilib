...
static bool DecodeImage_LOTTIE(
    sk_sp<skottie::Animation> &pSkAnimation,
...
bool Image_LOTTIE::LoadImageFile(
    ...
    bool bAssertEnabled)
{
    ...
    if (!m_impl->m_fileData.empty()) {
        m_impl->m_pSkAnimation
            = skottie::Animation::Builder()
                  .setFontManager(*pSkFontMgr)
                  .make((const char *) m_impl->m_fileData.data(), m_impl->m_fileData.size());
    } else {
        std::string jsonFilePath = imageFilePath.NativePathA();
        m_impl->m_pSkAnimation = skottie::Animation::Builder()
                                     .setFontManager(*pSkFontMgr)
                                     .makeFromFile((const char *) jsonFilePath.c_str());
    }
...
bool Image_LOTTIE::ReadFrameData(
    int32_t nFrameIndex, const UiSize &szDestRectSize, AnimationFrame *pAnimationFrame)
{
    ...
    if ((szDestRectSize.cx > 0) || (szDestRectSize.cy > 0)) {
        ...
        if ((szDestRectSize.cx > 0) && (szDestRectSize.cy > 0)) {
            if ((szDestRectSize.cx < (int32_t) nImageWidth)
                && (szDestRectSize.cy < (int32_t) nImageHeight)) {
    ...
    if (m_impl->m_pSkAnimation != nullptr) {
        if (DecodeImage_LOTTIE(m_impl->m_pSkAnimation, nImageWidth, nImageHeight, nFrameIndex, frame)) {
            if (frame != nullptr) {
