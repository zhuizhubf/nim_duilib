    ...
    for (const auto &attribute : attributeList) {
        ...
        if (name == _T("file") || name == _T("res")) {
            ...
            if (!value.empty()) {
                ...
                rect.left = StringUtil::StringToInt32(value.c_str(), &pstr, 10);
                ASSERT(pstr);
...
UiRect ImageAttribute::GetImageDestRect(
    int32_t imageWidth, int32_t imageHeight, const DpiManager &dpi) const
{
    UiRect rc;
    if (m_rcDest != nullptr) {
        rc = *m_rcDest;
...
void ImageAttribute::SetImageMargin(
    const UiMargin &newMargin, bool bNeedDpiScale, const DpiManager &dpi)
{
...
bool ImageAttribute::HasImageCorner() const
{
    ...
    return (rcCorner.left > 0) || (rcCorner.top > 0) || (rcCorner.right > 0)
           || (rcCorner.bottom > 0);
}
 ...
 */
static UiSize CalculateAdaptiveSize(
    int32_t nImageWidth, int32_t nImageHeight, const UiSize &targetSize)
{
    if ((nImageWidth <= 0) || (nImageHeight <= 0) || (targetSize.cx <= 0) || (targetSize.cy <= 0)) {
        return UiSize();
    }
...
UiRect ImageAttribute::CalculateAdaptiveRect(
    int32_t nImageWidth,
    ...
    const DString &vAlign)
{
    ...
    UiSize newSize
        = CalculateAdaptiveSize(nImageWidth, nImageHeight, UiSize(targetWidth, targetHeight));
