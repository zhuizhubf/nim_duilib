...
uint32_t ImageUtil::GetScaledImageSize(
    uint32_t nImageSize, uint32_t nNewDpiScale, uint32_t nOrgDpiScale)
{
    if ((nNewDpiScale != nOrgDpiScale) && (nOrgDpiScale != 0)) {
        nImageSize = static_cast<uint32_t>(std::ceil(
            nImageSize * static_cast<float>(nNewDpiScale) / static_cast<float>(nOrgDpiScale)));
    }
    return nImageSize;
}

bool ImageUtil::FlipPixelBits(
...
bool ImageUtil::ResizeImageData(
...
std::unique_ptr<IBitmap> ImageUtil::ResizeImageBitmap(
...
bool ImageUtil::GetBestImageScale(
