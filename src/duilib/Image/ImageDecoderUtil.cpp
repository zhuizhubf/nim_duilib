...
/** 从内存数据加载图片
    */
static bool LoadImageFromMemoryOrFile(
    const std::vector<uint8_t> &fileData, const FilePath &filePath, UiImageData &imageData)
{
    ...
    if (!fileData.empty()) {
        ...
        rgbaData = stbi_load_from_memory(
            buffer, len, &nWidth, &nHeight, &channels_in_file, desired_channels);
    } else if (!filePath.IsEmpty()) {
        std::string filePathA = filePath.NativePathA();
        rgbaData
            = stbi_load(filePathA.c_str(), &nWidth, &nHeight, &channels_in_file, desired_channels);
    } else {
        ASSERT(0);
    }
    ...
    if (((channels_in_file == 3) || (channels_in_file == 4)) && (nWidth > 0) && (nHeight > 0)) {
...
static bool LoadImageFromMemory(
    ...
    std::vector<UiImageData> &imageData)
{
    ...
    for (int32_t index = 0; index < frameCount; ++index) {
        ...
        cxIcoStream
            = std::make_unique<CxMemFile>((uint8_t *) fileData.data(), (uint32_t) fileData.size());
        ...
        for (lPy = 0; lPy < (int32_t) nHeight; ++lPy) {
            for (lPx = 0; lPx < (int32_t) nWidth; ++lPx) {
                *pBit = cxFrame->GetPixelColor(lPx, lPy, true);
                if (!cxFrame->AlphaIsValid() && !cxFrame->IsTransparent()
                    && !cxFrame->AlphaPaletteIsEnabled()) {
                    ...
                    int32_t transIndex
                        = cxFrame->GetTransIndex(); //Gets the index used for transparency. Returns -1 for no transparancy.
                    int32_t bitCount = cxFrame->GetBpp();        //1, 4, 8, 24.
                    int32_t numColors = cxFrame->GetNumColors(); //2, 16, 256; 0 for RGB images.
                    if ((transIndex >= 0) && (bitCount < 24) && (numColors != 0)
                        && (cxFrame->GetDIB() != nullptr)) {
    ...
    if (!bLoadAllFrames && !imageData.empty()) {
        ...
        for (auto color : colors) {
            for (size_t i = 0; i < imageCount; ++i) {
                ...
                if ((!isIconSizeValid || (icoData.m_imageWidth == nBestIconSize))
                    && (numColors == color)) {
...
bool ImageDecoderUtil::LoadImageFromFile(const FilePath &filePath, UiImageData &imageData)
{
    std::vector<uint8_t> fileData;
    return STBImageLoader::LoadImageFromMemoryOrFile(fileData, filePath, imageData);
}

bool ImageDecoderUtil::LoadImageFromMemory(
    const std::vector<uint8_t> &fileData, UiImageData &imageData)
{
    FilePath filePath;
    return STBImageLoader::LoadImageFromMemoryOrFile(fileData, filePath, imageData);
}

bool ImageDecoderUtil::LoadIcoFromFile(
...
bool ImageDecoderUtil::LoadIcoFromMemory(
    const std::vector<uint8_t> &fileData,
    bool bLoadAllFrames,
    uint32_t iconSize /*仅当bLoadAllFrames为false时有效*/,
    std::vector<UiImageData> &imageData)
