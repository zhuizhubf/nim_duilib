    ...
    if (sizeof(CharType) == 1) {
        ...
        while (i < nLen) {
            ...
            if ((ch & 0x80) == 0) {
                ...
                ch = ((ch & 0x07) << 18) | ((pData[i++] & 0x3F) << 12) | ((pData[i++] & 0x3F) << 6) | (pData[i++] & 0x3F);
                text.push_back(ch);
            }
        }
    }
...
std::vector<LayoutGlyph> BuildGlyphs(ITextShaper& textShaper,
                                     size_t nRunIndex,
                                     IFont* pFont,
                                     UiColor textColor,
...
std::vector<LayoutLine> WrapLines(const std::vector<LayoutGlyph>& glyphs,
                                  int32_t nMaxWidth,
                                  bool bSingleLine,
                                  float fLineHeight,
                                  float fWordSpacing)
...
void AppendEllipsis(ITextShaper& textShaper,
                    std::vector<LayoutGlyph>& glyphs,
                    IFont* pFont,
                    UiColor textColor,
                    float fWidth)
                  ...
                  RichTextLineInfoParam* pLineInfoParam)
{
    ...
    for (size_t i = 0; i < lines.size(); ++i) {
        ...
        DStringW lineText;
        pRowInfo->m_rowRect = UiRectF((float)textRect.left, (float)textRect.top + fLineHeight * (float)i,
...
void BuildRichLayout(ITextShaper& textShaper,
                     const UiRect& textRect,
                     ...
                     RichLayoutResult& result)
{
    ...
    for (size_t i = 0; i < richTextData.size(); ++i) {
        ...
        const float fRunLineHeight = metrics.m_fHeight * data.m_fRowSpacingMul + data.m_fRowSpacingAdd;
        result.m_fLineHeight = std::max(result.m_fLineHeight, fRunLineHeight);
        const std::vector<uint32_t> text = ToUTF32(DString(data.m_textView.data(), data.m_textView.size()));
        for (uint32_t ch : text) {
            if (IsLineBreakChar(ch)) {
                LayoutGlyph glyph;
                glyph.m_nRunIndex = i;
                                 ...
                                 const MeasureStringParam& measureParam)
{
    ...
    const std::vector<LayoutGlyph> glyphs = BuildGlyphs(textShaper, 0, measureParam.pFont, UiColor(), strText, drawParam);
...
void TextLayout::DrawString(ITextShaper& textShaper,
                            IRender* pRender,
                            const DString& strText,
                            const DrawStringParam& drawParam)
{
    ...
    const std::vector<LayoutGlyph> glyphs = BuildGlyphs(textShaper, 0, drawParam.pFont, drawParam.dwTextColor, strText, drawParam);
    float fLineHeight = 0.0f;
    float fFontHeight = 0.0f;
    ResolveLineHeight(textShaper, drawParam, fLineHeight, fFontHeight);
...
void TextLayout::MeasureRichText(ITextShaper& textShaper,
                                 const UiRect& textRect,
...
void TextLayout::MeasureRichText2(ITextShaper& textShaper,
                                  const UiRect& textRect,
                                  const UiSize& szScrollOffset,
                                  const std::vector<RichTextData>& richTextData,
                                  RichTextLineInfoParam* pLineInfoParam,
...
void TextLayout::DrawRichText(ITextShaper& textShaper,
                              IRender* pRender,
                              const UiRect& textRect,
                              const UiSize& /*szScrollOffset*/,
                              const std::vector<RichTextData>& richTextData,
                              ...
                              std::vector<std::vector<UiRect>>* pRichTextRects)
{
    ...
    for (const LayoutLine& line : result.m_lines) {
        for (const LayoutGlyph& glyph : line.m_glyphs) {
            textShaper.DrawGlyph(glyph.m_glyph, (float)textRect.left + glyph.m_x, y + result.m_fLineHeight * 0.8f,
                                 glyph.m_textColor, uFade);
        }
